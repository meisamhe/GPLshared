...
%path = 'C:\Users\MHE\Desktop\ActiveCourses\';
path ='E:\';
rp     = 3+n-1; % since here it is OL, Rating, version, weekend [only include weekend]
rq    = 4+n-1; % since both variance of rating and observational learning to explain heterogeneity in q
k     =  2*n+rp+rq; % (n)alpha fixed effect +  terms for p terms for q + (n)delta fixed effect
...
totalNomQueue = num(:,9)/100; %total nomination in queue
...
qpparam = 0.0131;
eta  = 0.0001;
pqM0beta = [max(yp)/3 ppparam qpparam eta npparam']; 
GTp  = diag(pqM0beta);
...
pqM0betaNewtemp  = zeros(parallelsize,kp);
 ...
 v      =  ones(1,n)*0.2;      w = 0.5;% w = 0.5*ones(1,n);              % known observation and sytem variance
 b0    =  zeros(k,1); %b0    =  zeros(k,n); 
 S0Inv =  1.5e-7*eye(k);
...
%  end;
 %pqalphaaddon   = [0.01;1e-3.*ones(k-1,1)];
 pqalphaaddon   = [0.01*ones(n,1);1e-3.*ones(k-n,1)];
 %get the size
 jtemp=1;
% consider that variables should only have relevant fixed effect
alphaMaddon = pqalphaaddon(jtemp);
deltaaddon  = pqalphaaddon(n+rp+rq+jtemp);

%only select relevant for current addon (relevant fixed effect
%and data coeffiencts)
betaaddon   = [pqalphaaddon(n+jtemp) pqalphaaddon(n+n+1:n+n+2)'];
gammaaddon  = [pqalphaaddon(rp+jtemp+n) pqalphaaddon(n+rp+n+1:n+rp+n+3)'];

btaddon = [-betaaddon';(-(1-deltaaddon).*gammaaddon./alphaMaddon)';((1-deltaaddon).*gammaaddon)';-deltaaddon];
    
 GT  = ones(size(btaddon(:,1),1),size(btaddon(:,1),1),n); %1 for last element of delta
...
%      alphaaparam = pqalphaaddon(1); % since alpha is between zero and 1
%      phetrgnprm  = pqalphaaddon(2:rp+1);
%      qhtrgnprm   = pqalphaaddon(rp+2:rp+rq+1);
%      deltaprm    = pqalphaaddon(rp+rq+2); %disadoption rate
   ...
   btaddon = [-betaaddon';(-(1-deltaaddon).*gammaaddon./alphaMaddon)';((1-deltaaddon).*gammaaddon)';-deltaaddon];%-qparamaddon/alphaMaddon;(1-pparamaddon+qparamaddon);
    ...
    GT   (:,:,jtemp)       =   diag(btaddon(:,1));   
     
     
     alphabetaapram{jtemp} =alphaMaddon.*betaaddon;
...
% to create fixed effect for both p and q
Ttotal = sum(T);
tempcounter = 1;
alphadeltafe     = zeros(Ttotal,n);
phetrgntotal= zeros(Ttotal,rp+2*n-2); % for the 2 elements as I have fiexed effect for alpha and gamma I would need seperate columns
qhetrgntotal= zeros(Ttotal,rq+3*n-3); % for the 3 elements as I have fiexed effect for alpha and gamma I would need seperate columns
xaddontotal = ones(Ttotal,1);
for i=1:n;
    % including fixed effect
    phetrgntotal(tempcounter:(tempcounter+T(i)-1),i)=ones(T(i),1);
    qhetrgntotal(tempcounter:(tempcounter+T(i)-1),i)=ones(T(i),1);
    
    % put real variables in
%     phetrgntotal(tempcounter:(tempcounter+T(i)-1),n+1:rp)= phetrgn{i}(:,2:rp-n+1);
%     qhetrgntotal(tempcounter:(tempcounter+T(i)-1),n+1:rq)= qhetrgn{i}(:,2:rq-n+1);
    % for two elements of p
    phetrgntotal(tempcounter:(tempcounter+T(i)-1),n+i)= phetrgn{i}(:,2); % the first n elements are fixed effects
    phetrgntotal(tempcounter:(tempcounter+T(i)-1),2*n+i)= phetrgn{i}(:,rp-n+1);
    % for two elements of q
    qhetrgntotal(tempcounter:(tempcounter+T(i)-1),n+i)= qhetrgn{i}(:,2);
    qhetrgntotal(tempcounter:(tempcounter+T(i)-1),2*n+i)= qhetrgn{i}(:,3);
    qhetrgntotal(tempcounter:(tempcounter+T(i)-1),3*n+i)= qhetrgn{i}(:,4);
    
    % for alpha and delta fixed effect for add-on i
    alphadeltafe(tempcounter:(tempcounter+T(i)-1),i)    = ones(T(i),1);
    
    % update the starting of window to fill
    tempcounter = tempcounter + T(i);
    
end;

% to speed up use sparase matrix
phetrgntotal=sparse(phetrgntotal);
qhetrgntotal=sparse(qhetrgntotal);

alphadeltafe     = sparse(alphadeltafe);
...
CSINT =zeros(n,csk);
 ...
 %alphaitems = [7,28,29,33,34,35,36];
 %alphaitems = [4,5,6,19,20,21,22,23,24,35,36];
...
% p0itmes = [7,14,33,34,35,36];
 %p0itmes = [3,4,5,6,7,11,12,13,35,36];
 p0itmes = [11,12,37,38,39];
 p1itmes = [7,15,22,34,35,36];
 p2itmes = [7,20,24,34,35,36];
 p3itmes = [7,30,34,35,36];
 p4itmes = [7,24,30,34,35,36];
 ...
 %qitems=[3,4,5,6,7];
 %q0items=[11,12,19,29,30,31];
 %q0items=[3,4,5,6,7,30,31];
 q0items=[32,33,34,35,36,37];
 q1items=[9,26,28,29,30,31];
 q2items=[11,29,30,31];
 q3items=[11,29,30,31];
 q4items=[10,21,29,30,31];
 q5items=[11,27,29,30,31];
 ...
 %deltaitems =  [4,33,34,35,36];
 %deltaitems =  [4,5,6,19,20,21,22,23,24,35,36];
...
% CSd =ones(n,1);
 ...
 cs6    =  zeros(7,1);
  ...
  %size 11
%  CS11Inv = 1.5e-7*eye(11);
%  cs11    =  zeros(11,1);
% intercept only
% CS11Inv = 1.5e-7*eye(1);
% cs11    =  zeros(1,1);

% only compensation and license
 CS11Inv = 1.5e-7*eye(6);
 cs11    =  zeros(6,1);
...
%   cs8    =  zeros(1,1);
 ...
 addonaccptcntr = 0;
 addonaccptcntralpha = 0;
 addonaccptcntrdelta = 0;
...
%dcs_=zeros(size([alphacsp0(:); alphacsq0(:); etacsp';etacsq'],1),((ndraw-ndraw0)/jumps)); 
dcs_ = zeros(29,((ndraw-ndraw0)/jumps)); 
%[alphacsalpha0(:);alphacsp0(:); alphacsq0(:);alphacsd0(:); etacsalpha(1);etacsp(1);etacsq(1);etacsd(1)];
...
%  end;
 pcaddonNonTrunc = 1e-5;%temppcaddon(i);%3.5e-5; %tunning parameter;
 pcaddonalpha = 1e-11;
 pcaddondelta = 1e-11;
 %excluded
%  
...
%  
 % definition of fixed effect variables
 ttr1addontotal =zeros(Ttotal,1);
  ttr1addon=zeros(Ttotal,1);
  mtotal=zeros(Ttotal,1);
  ttptotal=zeros(Ttotal,1);
  tttotal=zeros(Ttotal,1);
  tttotaldiag=zeros(Ttotal,n); %diagonal element in order to handle 1-delta
...
for i = 1:ndraw
   ...
   % simulate observation equation parameter (which is state equation for
    ...
    parfor j=1:n;
        ...
        [m{j},C{j},m0{j},C0{j},tt1{j},tt0{j},Y1{j},MAD{j},MSE{j}]     =     BEKFAddonpq(y{j},F,p,m0{j},C0{j},v(j),w,GT(:,:,j),uT{j}',j,pT,tt1p,tt0p,phetrgn{j},qhetrgn{j},rp-n+1,rq-n+1);   %,a,b,alpha,piaitemp(j),mp(:,1:pT),m0p(:,:)
        ttr1addon=tt1{j}';
    ...
    % No heterogeneity, but with fixed effect implemented through
    % hierarchical Bayesian
    %=======================================================================================
     % prepare for platform EKF  
     %creating the lag of responses
     tempcounter =1;
     for j=1:n;
        ttr1addontotal(tempcounter:(tempcounter+T(j)-1))=tt1{j}';        
        mtotal(tempcounter:(tempcounter+T(j)-1))=[m0{j};m{j}(1:T(j)-1)'];
        ttptotal(tempcounter:(tempcounter+T(j)-1))=[tt0p;tt1p(pT-T(j)+1:pT-1)'];
        tttotal(tempcounter:(tempcounter+T(j)-1))=[tt0{j};tt1{j}(1:T(j)-1)'];
        % diagonal for the case that we only have 1-delta
        tttotaldiag(tempcounter:(tempcounter+T(j)-1),j)=[tt0{j};tt1{j}(1:T(j)-1)'];
        tempcounter = tempcounter + T(j);
        %------------------------------->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
     end;
        tttotaldiag=sparse(tttotaldiag);
        ttind1addon= addonttindepbuilderOLSTpqFxdEffect(tttotal,xaddontotal,ttptotal,mtotal,phetrgntotal,qhetrgntotal,tttotaldiag) ; % drop last value
        ...
        ratekeepraddon =0;
        ratekeeperofwhile =0;
        ratekeeperofwhilealpha =0;
        ratekeeperofwhiledelta =0;
        while (alphaaddon < accptnrndaddon) 
%             disp('once.....................');
           ratekeepraddon=ratekeepraddon+1;          
           disp(ratekeepraddon);
           % draw elements that are not truncated normal
           pqalphaaddonNew(n+1:n+rp+rq,1) = pqalphaaddon(n+1:n+rp+rq,1)+ pcaddonNonTrunc*randn(length(pqalphaaddon(n+1:n+rp+rq)),1);
           % draw alpha truncated normal
           pqalphaaddonNew(1:n,1) = pqalphaaddon(1:n,1) + pcaddonalpha*randn(length(pqalphaaddon(1:n)),1);
           while   (sum(pqalphaaddonNew(1:n,1)<0)>0) ||  (sum(pqalphaaddonNew(1:n,1)>1)>0) 
               %ratekeepraddon=ratekeepraddon+1;        
                pqalphaaddonNew(n+1:n+rp+rq,1) = pqalphaaddon(n+1:n+rp+rq,1)+ pcaddonalpha*randn(length(pqalphaaddon(n+1:n+rp+rq)),1);
               ratekeeperofwhilealpha=ratekeeperofwhilealpha+1
               if (ratekeeperofwhilealpha>1e3) 
                   if (floor(ratekeeperofwhilealpha./100)==ratekeeperofwhilealpha/100) 
                        if (1/ratekeeperofwhilealpha<0.001) 
                              pcaddonalpha =  pcaddonalpha*3; % because it is not about log likelihood but for a number between 0 and 1
                        end;
                         ratekeeperofwhilealpha =0;
                   end;
               end;
           end;
%            addonaccptcntralpha = addonaccptcntralpha + ratekeeperofwhilealpha;
%            if (i/addonaccptcntralpha>0.15) && (floor(i./5)==i/5)
%                   pcaddonalpha =  pcaddonalpha*3;
%                   addonaccptcntralpha = i./0.15;% to maintain past dependancy 
%            end;
           pqalphaaddonNew(n+rp+rq+1:n+rp+rq+n,1) = pqalphaaddon(n+rp+rq+1:n+rp+rq+n,1) + pcaddondelta*randn((length(n+rp+rq+1:n+rp+rq+n)),1);
           while    (sum(pqalphaaddonNew(n+rp+rq+1:n+rp+rq+n)<0,1)>0) ||  (sum(pqalphaaddonNew(n+rp+rq+1:n+rp+rq+n,1)>1)>0)
               %ratekeepraddon=ratekeepraddon+1;  
               ratekeeperofwhiledelta = ratekeeperofwhiledelta+1;
               pqalphaaddonNew(n+rp+rq+1:n+rp+rq+n,1) = pqalphaaddon(n+rp+rq+1:n+rp+rq+n,1) + pcaddondelta*randn((length(n+rp+rq+1:n+rp+rq+n)),1);          ratekeeperofwhiledelta=ratekeeperofwhiledelta+1
                if (ratekeeperofwhiledelta>1e3) 
                   if (floor(ratekeeperofwhiledelta./100)==ratekeeperofwhiledelta/100) 
                        if (1/ratekeeperofwhiledelta<0.001)  
                             pcaddondelta =  pcaddondelt*3;   % because it is not about log likelihood but for a number between 0 and 1                    
                        end;
                        ratekeeperofwhiledelta =0;
                   end;
               end;
           end;
%             addonaccptcntrdelta = addonaccptcntrdelta + ratekeeperofwhiledelta;
%            if (i/addonaccptcntrdelta>0.15) && (floor(i./5)==i/5)
%                   pcaddondelta =  pcaddondelta*3;
%                   addonaccptcntrdelta = i./0.15;% to maintain past dependancy 
%            end;
           postPNewaddon  = MHLLSROLRTpqFE(pqalphaaddonNew,ttind1addon,ttr1addontotal,Ttotal,w,b0,etacs,rp,rq,n); % log of new posterior
           postPOldaddon  = MHLLSROLRTpqFE(pqalphaaddon,ttind1addon,ttr1addontotal,Ttotal,w,b0,etacs,rp,rq,n); % log of old posterior
           alphaaddon    = postPNewaddon - postPOldaddon;
           accptnrndaddon = log(rand(1,1));
           if (ratekeepraddon>1e3)
               if (floor(ratekeepraddon./100)==ratekeepraddon/100) 
                    if (1/ratekeepraddon<0.001) 
                          pcaddonNonTrunc =  pcaddonNonTrunc/3;
                    end;
                    ratekeepraddon =0;
               end;
           end;
        ...
        % check acceptance rate and modify it
        addonaccptcntr = addonaccptcntr + ratekeepraddon;
        accepRateaddon=i/addonaccptcntr % because i out of all cumj iterations is accepted until now
       if (floor(i./5)==i/5)
            if (accepRateaddon>0.15)  
                 pcaddonNonTrunc =  pcaddonNonTrunc*3;
                 addonaccptcntr = i./0.15;% to maintain past dependancy 
            end;
       end;
        disp(addonaccptcntr)
        disp(accepRateaddon)
...
%         qparamaddon     = pqalphaaddon{j}(2);
        for j=1:n
            ...
            % consider that variables should only have relevant fixed effect              
            alphaMaddon =pqalphaaddon(j);
            %exp(pqalphaaddon(j)-log(1+exp(pqalphaaddon(j))));
            deltaaddon  = pqalphaaddon(n+rp+rq+j);
            %exp(pqalphaaddon(n+rp+rq+j)-log(1+exp(pqalphaaddon(n+rp+rq+j))));
            
            %only select relevant for current addon (relevant fixed effect
            %and data coeffiencts)
            betaaddon   = [pqalphaaddon(j+n) pqalphaaddon(n+n+1:n+n+2)'];
            gammaaddon  = [pqalphaaddon(rp+j+n) pqalphaaddon(n+rp+n+1:n+rp+n+3)'];

            btaddon = [-betaaddon';(-(1-deltaaddon).*gammaaddon./alphaMaddon)';((1-deltaaddon).*gammaaddon)';-deltaaddon]; %-qparamaddon/alphaMaddon;(1-pparamaddon+qparamaddon);
            % btaddon = [-betaaddon;-gammaaddon/alphaMaddon;gammaaddon];
            
            disp('add-on j:')
            disp([j alphaMaddon betaaddon gammaaddon deltaaddon]) %pparamaddon qparamaddon
    ...
    %         disp([alphaMaddon betaaddon' gammaaddon']) %pparamaddon qparamaddon
      ...
      %----------------------------------------Recover back parameters for calculations-----------------------------------------------------------
        alphaMaddon = exp(pqalphaaddon(1:n)-log(1+exp(pqalphaaddon(1:n))));
        betaaddon  = pqalphaaddon(1+n:rp+n);
        gammaaddon  = pqalphaaddon(rp+n+1:rp+rq+n);
        deltaaddon  = exp(pqalphaaddon(rp+rq+n+1:rp+rq+n+n)-log(1+exp(pqalphaaddon(rp+rq+n+1:rp+rq+n+n))));
        btempaddon = [alphaMaddon;betaaddon;gammaaddon;deltaaddon]; %pparamaddon;qparamaddon;
        
        betaaddon   = [pqalphaaddon(1+n:2*n);pqalphaaddon(2*n+1)*ones(n,1);pqalphaaddon(2*n+2)*ones(n,1)]; %this will be fixed effects first and then replication of related elements
        gammaaddon  = [pqalphaaddon(rp+n+1:rp+2*n);pqalphaaddon(rp+2*n+1)*ones(n,1);pqalphaaddon(rp+2*n+2)*ones(n,1);pqalphaaddon(rp+2*n+3)*ones(n,1)];       
       ...
       % pqalphatempaddon = [-gammaaddon*(1-deltaaddon)/alphaMaddon;1-deltaaddon;-betaaddon;alphaMaddon.*betaaddon;(1-deltaaddon)*gammaaddon];
        pqalphatempaddon = [-gammaaddon.*repmat((1-deltaaddon)./alphaMaddon,[4 1]);1-deltaaddon;-betaaddon;repmat(alphaMaddon,[3 1]).*betaaddon;...
            repmat((1-deltaaddon),[4 1]).*gammaaddon];
      ...
      %  pqalphatempaddon = [-gammaaddon/alphaMaddon;1;-betaaddon;alphaMaddon.*betaaddon;gammaaddon];
        ...
        wa1  = (wa0+Ttotal)/2;
        wv12 = (wv02+(ttr1addontotal-ttind1addon(1:Ttotal,:)*pqalphatempaddon)'*(ttr1addontotal-ttind1addon(1:Ttotal,:)*pqalphatempaddon))/2;
        w   =  1/gamrnd(wa1,1/wv12);
        if (sw==1)
              c_          =     [btempaddon ;v(:);w];
              %as the sum is only important I am adding it once so adding
              %to the first element will not harm
               LLtemp (1)      =  LLtemp (1)- 0.5*(ttr1addontotal-ttind1addon(1:Ttotal,:)*pqalphatempaddon)'*(ttr1addontotal-ttind1addon(1:Ttotal,:)*pqalphatempaddon)/w - ...
                0.5*Ttotal*p*log(2*pi*w);
               errw_  =  errw_+(ttr1addontotal-ttind1addon(1:Ttotal,:)*pqalphatempaddon);
        end;
        % recover utility in order to regress
%         alphaMaddon = pqalphaaddon(1:n);
%         deltaaddon  = pqalphaaddon(rp+rq+n+1:rp+rq+n+n);
%         btempaddon = [alphaMaddon;betaaddon;gammaaddon;deltaaddon]; %pparamaddon;qparamaddon;
    %========================================================================================
     if (sw==1) 
          Y1a{jp}         =     Y1; 
   ...
   % estimation of plug in level effects
    ...
    b1   =  S1*(((CSAlpha'*btempaddon(1:n))/etacsalpha(j))+CSalphaInv*csalpha); %btempp(j,1)
    alphacsalpha0   =  b1 + chol(S1)'*randn(length(b1),1);
    wa1  = (wa0+n)/2;
    wv12 = (wv02+(btempaddon(1:n)-CSAlpha*alphacsalpha0)'*(btempaddon(1:n)-CSAlpha*alphacsalpha0))/2; %btempp(j,1)  btempp(j,1)
    ...
    S1   =  (((CSp0'*CSp0)/etacsp(j))+CS11Inv)\speye(size((((CSp0'*CSp0)/etacsp(j))+CS11Inv))); %inv(((CSL'*CSL)/eta(j))+CS0Inv); 
    b1   =  S1*(((CSp0'*btempaddon(n+1:2*n))/etacsp(j))+CS11Inv*cs11); %btempp(j,1)
    alphacsp0   =  b1 + chol(S1)'*randn(length(b1),1);
    wa1  = (wa0+n)/2;
    wv12 = (wv02+(btempaddon(n+1:2*n)-CSp0*alphacsp0)'*(btempaddon(n+1:2*n)-CSp0*alphacsp0))/2; %btempp(j,1) btempp(j,1)
    etacsp(j)   =  1/gamrnd(wa1,1/wv12);
    
%     j=2; % addon version
%     S1   =  (((CSp1'*CSp1)/etacsp(j))+CS6Inv)\speye(size((((CSp1'*CSp1)/etacsp(j))+CS6Inv)));% inv(((CSIntr'*CSIntr)/eta(j))+CS0Inv); 
%     b1   =  S1*(((CSp1'*btempaddon(j,:)')/etacsp(j))+CS6Inv*cs6); % btempp(j,1)
...
%     
%     j=3; % ffox version
%     S1   =  (((CSp2'*CSp2)/etacsp(j))+CS6Inv)\speye(size((((CSp2'*CSp2)/etacsp(j))+CS6Inv)));% inv(((CSSQ'*CSSQ)/eta(j))+CS0Inv); 
%     b1   =  S1*(((CSp2'*btempaddon(j,:)')/etacsp(j))+CS6Inv*cs6); %btempp(j,1)
...
%     etacsp(j)   =  1/gamrnd(wa1,1/wv12);
    ...
    %---------------------------------------------------------------
     ...
     j=1; % for version
    S1   =  (((CSq0'*CSq0)/etacsq(j))+CS8Inv)\speye(size((((CSq0'*CSq0)/etacsq(j))+CS8Inv))); %inv(((CSL'*CSL)/eta(j))+CS0Inv); 
    b1   =  S1*(((CSq0'*btempaddon(n+n+2+1:2*n+n+2))/etacsq(j))+CS8Inv*cs8); %btempp(j,1)
    alphacsq0   =  b1 + chol(S1)'*randn(length(b1),1);
    wa1  = (wa0+n)/2;
    wv12 = (wv02+(btempaddon(n+n+2+1:2*n+n+2)-CSq0*alphacsq0)'*(btempaddon(n+n+2+1:2*n+n+2)-CSq0*alphacsq0))/2; %btempp(j,1) btempp(j,1)
...
%     
%     j=3; % for version
%     S1   =  (((CSq2'*CSq2)/etacsq(j))+CS4Inv)\speye(size((((CSq2'*CSq2)/etacsq(j))+CS4Inv))); %inv(((CSL'*CSL)/eta(j))+CS0Inv); 
%     b1   =  S1*(((CSq2'*btempaddon(j,:)')/etacsq(j))+CS4Inv*cs4); %btempp(j,1)
...
%     S1   =  (((CSq4'*CSq4)/etacsq(j))+CS5Inv)\speye(size((((CSq4'*CSq4)/etacsq(j))+CS5Inv))); %inv(((CSL'*CSL)/eta(j))+CS0Inv); 
%     b1   =  S1*(((CSq4'*btempaddon(j,:)')/etacsq(j))+CS5Inv*cs5); %btempp(j,1)
...
%     etacsq(j)   =  1/gamrnd(wa1,1/wv12);  
    ...
    b1   =  S1*(((CSd'*btempaddon(3*n+6:4*n+5))/etacsd(j))+CSdInv*csd0); %btempp(j,1)
    alphacsd0   =  b1 + chol(S1)'*randn(length(b1),1);
    wa1  = (wa0+n)/2;
    wv12 = (wv02+(btempaddon(3*n+6:4*n+5)-CSd*alphacsd0)'*(btempaddon(3*n+6:4*n+5)-CSd*alphacsd0))/2; %btempp(j,1) btempp(j,1)
    etacsd(j)   =  1/gamrnd(wa1,1/wv12);   
% %    
    % updating prior for cross sectional effects
%     for j=1:n;
%         %b0(:,j)  =   [CSSQ(j,:)*alpha(:,1); CSL(j,:)*alpha(:,2); CSIntr(j,:)*alpha(:,3)];
%          alphaparamtemp = CSAlpha(j,:)*alphacsalpha0;
% %         pparamtemp     = CSL(j,:)*alphacs(:,2);
% %         qparamtemp     = CSIntr(j,:)*alphacs(:,3);
%         % for p0
%         betaparamtemp(1)  = CSp0(j,:)* alphacsp0;
%         % for p1
%         betaparamtemp(2)  = 0.01;%CSp1(j,:)* alphacsp1;
%         % for p2
%         betaparamtemp(3)  = 0.1;
%         % for p3
...
%         % for q1
%         gammaparamtemp(2)  = -0.1;
%          % for q2
%         gammaparamtemp(3)  = 0.1;
%          % for q3
% %        gammaparamtemp(4)  = CSq3(j,:)* alphacsq3;
%          % for q4
%         gammaparamtemp(4)  = 0.1;
%          % for q5
...
% %         end;
%         deltaparamtemp  = CSd(j,:)*alphacsd0;%; CSINT(j,:)*alpha(6)]; ;CSST(j,:)*alphacs(6)
%         b0(:,j)   = [alphaparamtemp;betaparamtemp';gammaparamtemp';deltaparamtemp]; %;pparamtemp;qparamtemp
%     end;
    %prior in the form of [alpha;beta;gamma;delta]
    b0   = [CSAlpha*alphacsalpha0;CSp0*alphacsp0;0.01;01;CSq0* alphacsq0;-01;0.1;0.1;CSd*alphacsd0]; %;pparamtemp;qparamtemp
    etacs = [etacsalpha';etacsp';etacsq';etacsd'];

    if (sw==1)
         dcs_(:,jp)          =    [alphacsalpha0(:);alphacsp0(:); alphacsq0(:);alphacsd0(:); etacsalpha(1);etacsp(1);etacsq(1);etacsd(1)];
%          [alphacsalpha(:);alphacsp0(:);alphacsp1(:);alphacsp2(:);...
...
k     =  2*n+rp+rq; % alpha +  terms for p terms for q %delta+ removed
% add-on parameters (n+1 variance: n of v, and 1 w)
%c_temp= zeros(k+1+n,(ndraw-ndraw0)/jumps,n);
c_temp= zeros(k+1+n,(ndraw-ndraw0)/jumps);
...
parfor  j=1:n;
    ...
    LLtemp (j)      =   - 0.5*T(j)*p*log(2*pi*ce_(k+j).^2) - 0.5* errv_{j}'*errv_{j}/ce_(k+j) ;
end;
errw_ = errw_/((ndraw-ndraw0)/jumps);
% consier platform
LLmean     = LLmean + sum(LLtemp)- 0.5*(errw_'*errw_)/ce_(k+1+n)- ...
        0.5*T(j)*p*log(2*pi*ce_(k+1+n));
