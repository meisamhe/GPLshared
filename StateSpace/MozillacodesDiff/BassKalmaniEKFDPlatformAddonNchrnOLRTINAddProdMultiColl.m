...
rq    = 6; % since both variance of rating and observational learning to explain heterogeneity in q
...
qpparam = 0.0131;
eta  = 0.0001;
pqM0beta = [max(yp)/5 ppparam qpparam eta npparam']; 
GTp  = diag(pqM0beta);
...
[numCatOL,txtCatOL,rawCatOL] = xlsread('C:\Users\MHE\Desktop\ActiveCourses\Projects\FirefoxAddon(VeryLowPriority)\DiffData\AddonCategoryLevelOLVar.xlsx',1);
[numCatVAR,txtCatVAR,rawCatVAR] = xlsread('C:\Users\MHE\Desktop\ActiveCourses\Projects\FirefoxAddon(VeryLowPriority)\DiffData\AddonCategoryLevelOLVar.xlsx',2);
parfor i=1:n;
    i
  ...
  %  T(1,i)=min(min(checksize(num(:,15)),checksize(num(:,1))),checksize(num(:,8)));
   ...
   % beta = 0.0001*rand(1,1);
    ...
    qhetrgn{i} =[stv{i} ol{i} stv{i}.*ol{i} stavg{i} stavg{i}.*stv{i}]; %status{i}
...
CSINT =zeros(n,csk);
 ...
 % cross sectional explanation of alpha
...
%         disp([alphaMaddon betaaddon' gammaaddon']) %pparamaddon qparamaddon
   ...
   % estimation of plug in level effects
    ...
    S1   =  (((CSAlpha'*CSAlpha)/etacs(j))+CSalphaInv)\speye(size((((CSAlpha'*CSAlpha)/etacs(j))+CSalphaInv)));% inv(((CSSQ'*CSSQ)/eta(j))+CS0Inv); 
    b1   =  S1*(((CSAlpha'*btempaddon(j,:)')/etacs(j))+CSalphaInv*csbalpha); %btempp(j,1)
    alphacs(:,j)   =  b1 + chol(S1)'*randn(length(b1),1);
    wa1  = (wa0+n)/2;
    wv12 = (wv02+(btempaddon(j,:)'-CSAlpha*alphacs(:,j))'*(btempaddon(j,:)'-CSAlpha*alphacs(:,j)))/2; %btempp(j,1)  btempp(j,1)
    ...
    etacs(j)   =  1/gamrnd(wa1,1/wv12);
      ...
      % estimation of plug in level effects
    j=4; %for carryover
    S1   =  (((CSAlpha'*CSAlpha)/etacs(j))+CS0Inv)\speye(size((((CSAlpha'*CSAlpha)/etacs(j))+CS0Inv)));% inv(((CSSQ'*CSSQ)/eta(j))+CS0Inv); 
    b1   =  S1*(((CSAlpha'*btempaddon(j,:)')/etacs(j))+CS0Inv*csb0); %btempp(j,1)
    alphacs(:,j)   =  b1 + chol(S1)'*randn(length(b1),1);
    wa1  = (wa0+n)/2;
    wv12 = (wv02+(btempaddon(j,:)'-CSAlpha*alphacs(:,j))'*(btempaddon(j,:)'-CSAlpha*alphacs(:,j)))/2; %btempp(j,1)  btempp(j,1)
    etacs(j)   =  1/gamrnd(wa1,1/wv12);
...
% %    
    ...
    for j=1:n;
        ...
        alphaparamtemp = CSAlpha(j,:)*alphacs(:,1);
...
%         qparamtemp     = CSIntr(j,:)*alphacs(:,3);
        ...
        deltaparamtemp  = [CSAlpha(j,:)*alphacs(:,2+rp+rq)];%; CSINT(j,:)*alpha(6)]; ;CSST(j,:)*alphacs(6)
