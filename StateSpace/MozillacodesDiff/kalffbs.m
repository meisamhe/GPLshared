...
function [m,C,m0,C0,tt1,tt0] = kalffbs(y,F,p,m0,C0,v,w,GT,uT) %,b1,b2,alpha
% Author:  Norris Bruce... June 2013
global Y1 MAD MSE 

 T   =  length(y); m = repmat(m0,[1 T]); C = double(repmat(C0,[1 1 T]));   C(:,:,1)=C0;  m(:,1)= m0; d= diag(GT);
 mt  =  m0; Ct = C0;    Y1 = [];   g  = GT;  MAD=[]; MSE=[];   % mt, Ct mean and variance of prior at t-1
 ...
 for t      =   1:T
    ...
    %forecast = F*a;  Y1(:,t)  = forecast;            % mean of 1-step ahead prediction
     forecast = ha(a); Y1(:,t)  = forecast;     % ha - function b1,b2,
     q      =   F*r*F' + v;                           % variance of 1-step ahead prediction
     
     A      =    r*F'*inv(q);   
     e      =   (y(:,t) - forecast); 
     MSE(t) =    sum(e.^2); MAD(t) = sum(abs(e));
     mt     =    a + A * e; 
     Ct = r - A*q*A'; 
                   
     Ct     =    (Ct+Ct')/2; C(:,:,t)=Ct;  m(:,t)= mt; % mean variance of posterior at t
