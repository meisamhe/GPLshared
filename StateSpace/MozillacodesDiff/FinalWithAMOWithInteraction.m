% File of heterogeneity in all the sensitivities [May 18, with AMO, with interaction]
...
rp     = 4; % since here it is OL, Rating, version, weekend [1 interaction term]
rq    = 6; % since both variance of rating and observational learning to explain heterogeneity in q [2 interaction terms]
...
parfor i=1:n;
  ...
  %  T(1,i)=min(min(checksize(num(:,15)),checksize(num(:,1))),checksize(num(:,8)));
   ...
   % beta = 0.0001*rand(1,1);
    [xaddon{i},y{i},uT{i},ol{i},stv{i},ver{i},wkend{i},status{i},T(i),stol{i},stavg{i}] = readOLRTInteraction(numaddon,T(i),alphabetaapram{i},p,tt1p,tt0p,pT,scale,pusg,ffxvrsn,numCatOL,numCatVAR,i); %int{i},piai{i}
    phetrgn{i} =[ver{i} ffxvrsn(pT-T(i)+1:pT) ver{i}.*ffxvrsn(pT-T(i)+1:pT)]; %ver{i}.*ffxvrsn(pT-T(i)+1:pT) wkend{i} wkend{i}
    ...
    qhetrgn{i} =[stv{i} ol{i}  stavg{i} stv{i}.*ol{i} stavg{i}.*stv{i}]; %status{i} %stv{i}.*ol{i} stavg{i}.*stv{i}
...
%  alphacs=zeros(csk,k); 
 ...
 dcs_=zeros(size( [alphacsalpha(:);alphacsp0(:);alphacsp1(:);alphacsp2(:);alphacsp3(:);...
      ...
      %  pqalphatempaddon = [-gammaaddon/alphaMaddon;1;-betaaddon;alphaMaddon.*betaaddon;gammaaddon];
        ...
        w (j)  =  1/gamrnd(wa1,1/wv12);
         ...
         if (sw==1);
           ...
           LLtemp (j)      = - 0.5*(ttr1addon-ttind1addon(1:T(j),:)*pqalphatempaddon)'*(ttr1addon-ttind1addon(1:T(j),:)*pqalphatempaddon)/w (j) - ...
...
%     etacsp(j)   =  1/gamrnd(wa1,1/wv12);
    ...
    S1   =  (((CSq3'*CSq3)/etacsq(j))+CS6Inv)\speye(size((((CSq3'*CSq3)/etacsq(j))+CS6Inv))); %inv(((CSL'*CSL)/eta(j))+CS0Inv); 
    b1   =  S1*(((CSq3'*btempaddon(j,:)')/etacsq(j))+CS6Inv*cs6); %btempp(j,1)
...
%     
    ...
    S1   =  (((CSq5'*CSq5)/etacsq(j))+CS6Inv)\speye(size((((CSq5'*CSq5)/etacsq(j))+CS6Inv))); %inv(((CSL'*CSL)/eta(j))+CS0Inv); 
    b1   =  S1*(((CSq5'*btempaddon(j,:)')/etacsq(j))+CS6Inv*cs6); %btempp(j,1)
...
%         qparamtemp     = CSIntr(j,:)*alphacs(:,3);
      ...
      %  betaparamtemp(5)  = CSp4(j,:)* alphacsp4;
        ...
        gammaparamtemp(4)  = CSq3(j,:)* alphacsq3;
         % for q4
        gammaparamtemp(5)  = CSq4(j,:)* alphacsq5;
         % for q5
...
%         end;
    ...
    if (sw==1)
         dcs_(:,jp)          =    [alphacsalpha(:);alphacsp0(:);alphacsp1(:);alphacsp2(:);alphacsp3(:);...
                 alphacsq0(:);alphacsq1(:);alphacsq2(:);alphacsq3(:);alphacsq4(:);alphacsq5(:);...
