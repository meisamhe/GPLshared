...
pqM0betaNewtemp  = zeros(parallelsize,kp);
 ...
 v      =  ones(1,n)*0.2;      
 w = 0.5;% w = 0.5*ones(1,n);              % known observation and sytem variance
...
disp([vp; wp])
 ...
 phetrgn =cell([1 n]);
 % postPOld = cell([1 n]);
 pqalphaaddon =  [0.01;1e-3.*ones(k-1,1)];%cell([1 n]);
...
alphaMaddon = pqalphaaddon(1);
betaaddon  = pqalphaaddon(2:rp+1);
gammaaddon  = pqalphaaddon(rp+2:rp+rq+1);
deltaaddon  = pqalphaaddon(rp+rq+2);
...
% pitmes = [3,4,5,6,7,11,12];
 ...
 addonaccptcntr = 0;% cell([1 n]); % keep track of counter of acceptance for addons
 ...
 c_        =    cell([1 n]);
 crosssec_ = cell([1 (ndraw-ndraw0)/jumps]);
...
ttr1addon = cell([1 n]);
ttind1addon = cell([1 n]);
StateLL = zeros(1,(ndraw-ndraw0)/jumps);
...
%         errvp_  =  errvp_ + evv;
   ...
   % simulate observation equation parameter (which is state equation for
    ...
    parfor j=1:n;
        ...
        [m{j},C{j},m0{j},C0{j},tt1{j},tt0{j},Y1{j},MAD{j},MSE{j}]     =     BEKFAddonpq(y{j},F,p,m0{j},C0{j},v(j),w,GT(:,:,j),uT{j}',j,pT,tt1p,tt0p,phetrgn{j},qhetrgn{j},rp,rq);   %,a,b,alpha,piaitemp(j),mp(:,1:pT),m0p(:,:)
        ...
        va1= (va0+T(j))/2;
        vv12= (vv02+(y{j}' - ttr1addon{j}')*(y{j}' - ttr1addon{j}')')/2;
        v(j) =1/gamrnd(va1,1/vv12);
        
        % prepare for platform EKF      
        ttind1addon{j}= addonttindepbuilderOLSTpq(tt0{j},tt1{j},T(j),xaddon{j},tt1p,tt0p,pT,m{j},m0{j},phetrgn{j},qhetrgn{j}) ; % drop last value
        
       if (sw==1);
            b_{j}          =     reshape(tt1{j}',T(j)*p,1); 
            b0_{j}         =     tt0{j}; 
            LLtemp (j)      =  -0.5*(y{j}' - ttr1addon{j}')*(y{j}' - ttr1addon{j}')'/v(j).^2 - ... %LLtemp (j)
            ...
            c_{j}          =     [v(j)];
             ...
             if (savinground ==1)
                ...
                m0LL{j}  =  m0LL{j}+m0{j};
                mLL{j}   = mLL{j}+m{j};
                ttr1addonLL{j} = ttr1addonLL{j}+ttr1addon{j};
                vLL(j) = vLL(j)+v(j);
             end;
     ...
     %#############################################################################################
     % Stack all and then find non-heterogeneous model
     %#############################################################################################
       % prepare data
       ttind1addonAll = cat(1,ttind1addon{:});
       ttr1addonAll=cat(1,ttr1addon{:});
...
%             disp('once.....................');
           ...
           disp(ratekeepraddon);
           pqalphaaddonNewAll = pqalphaaddon + pcaddon*randn(length(pqalphaaddon),1);
           while   (pqalphaaddonNewAll(1)<0) || (pqalphaaddonNewAll(1)>1) || (pqalphaaddonNewAll(rp+rq+2)<0) || (pqalphaaddonNewAll(rp+rq+2)>1) 
               ratekeepraddon=ratekeepraddon+1;
               pqalphaaddonNewAll = pqalphaaddon + pcaddon*randn(length(pqalphaaddon),1);
           ...
           postPNewaddon  = MHLLSROLRTpq(pqalphaaddonNewAll,ttind1addonAll,ttr1addonAll,sum(T),w,b0,etacs,rp,rq); % log of new posterior
           postPOldaddon  = MHLLSROLRTpq(pqalphaaddon,ttind1addonAll,ttr1addonAll,sum(T),w,b0,etacs,rp,rq); % log of old posterior
           alphaaddon    = postPNewaddon - postPOldaddon;
           accptnrndaddon = log(rand(1,1));
           if (ratekeepraddon>1e4)
               postPNewaddon = postPOldaddon;
               pqalphaaddonNewAll =pqalphaaddon;
               break;
           end;
        end;
        % check acceptance rate and modify it
       ...
       if (floor(i./5)==i/5)
        ...
        pqalphaaddon   = pqalphaaddonNewAll;  
...
%         qparamaddon     = pqalphaaddon{j}(2);
        ...
        wa1  = (wa0+sum(T))/2;
        wv12 = (wv02+(ttr1addonAll-ttind1addonAll(1:sum(T),:)*pqalphatempaddon)'*(ttr1addonAll-ttind1addonAll(1:sum(T),:)*pqalphatempaddon))/2;
        w   =  1/gamrnd(wa1,1/wv12);
      


        if (sw==1);
            StateLL(jp)        = - 0.5*(ttr1addonAll-ttind1addonAll(1:sum(T),:)*pqalphatempaddon)'*(ttr1addonAll-ttind1addonAll(1:sum(T),:)*pqalphatempaddon)/w  - ...
                                0.5*sum(T)*p*log(2*pi*w); % LLtemp (j)
            crosssec_{jp}          =     [btempaddon;w];
            
             if (savinground ==1)
                gammaaddonLL = gammaaddon;
                deltaaddonLL = deltaaddon;
                alphaMaddonLL = alphaMaddon;
                betaaddonLL = betaaddon;
                wLL = w;
             else
                gammaaddonLL =  gammaaddonLL+gammaaddon;
                deltaaddonLL = deltaaddonLL+deltaaddon;
                alphaMaddonLL = alphaMaddonLL+alphaMaddon;
                betaaddonLL = betaaddonLL+betaaddon;
                wLL = wLL + w ;
             end;
...
k     =  2+rp+rq; % alpha +  terms for p terms for q %delta+ removed
% add-on parameters
c_temp=zeros(1,(ndraw-ndraw0)/jumps,n);
...
cemeani_ = mean(ce_,2);
disp('parameter estiamte is (mean posterior): V');
disp(mean(ce_,2)');
...
wLL = wLL /((ndraw-ndraw0)/jumps); 
ttind1addonLL = cell([1 n]);
for  j=1:n;
    ...
    ttind1addonLL{j}= addonttindepbuilderOLSTpq(tt0LL{j},tt1LL{j},T(j),xaddon{j},ttr1LL',ttind1pLL(1),pT,mLL{j},m0LL{j},phetrgn{j},qhetrgn{j}) ; % drop last value
...
%     0.5*T(j)*p*log(2*pi*wLL(j)); % LLtemp (j)        
 ...
 gammaaddonLL = gammaaddonLL/((ndraw-ndraw0)/jumps);
 deltaaddonLL = deltaaddonLL/((ndraw-ndraw0)/jumps);
 alphaMaddonLL = alphaMaddonLL/((ndraw-ndraw0)/jumps);
 betaaddonLL = betaaddonLL/((ndraw-ndraw0)/jumps);
 pqalphatempaddonLL = [-gammaaddonLL*(1-deltaaddonLL)/alphaMaddonLL;1-deltaaddonLL;-betaaddonLL;alphaMaddonLL.*betaaddonLL;(1-deltaaddonLL)*gammaaddonLL];

 ttind1addonAllLL = cat(1,ttind1addonLL{:});
 ttr1addonAllLL=cat(1,ttr1addonLL{:});
 LLmean        = LLmean - 0.5*(ttr1addonAllLL-ttind1addonAllLL(1:sum(T),:)*pqalphatempaddonLL)'*(ttr1addonAllLL-ttind1addonAllLL(1:sum(T),:)*pqalphatempaddonLL)/w  - ...
                                0.5*sum(T)*p*log(2*pi*w); % LLtemp (j)
