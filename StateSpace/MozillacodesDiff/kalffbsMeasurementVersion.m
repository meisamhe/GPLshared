...
function [m,C,m0,C0,tt1] = kalffbsMeasurementVersion(y,F,p,m0,C0,v,w,GT,uT)
...
global Y1 MAD MSE
 ...
 for t      =   1:T
     ...
     a      =   g*mt + uT;                                 % mean of prior at t-1
     
     %pool forecasts
     
     forecast = reshape(F(t,:,:),2,3)'*a;  Y1(t,:)  = forecast;       % mean of 1-step ahead prediction
     q      =  reshape(F(t,:,:),2,3)'*r*reshape(F(t,:,:),2,3) + v;                           % variance of 1-step ahead prediction
     
     A      =    r*reshape(F(t,:,:),2,3)*(q\speye(size(q)));                     % fraction of variance of state to variance of data
     e      =   (y(t,:) - forecast')'; 
