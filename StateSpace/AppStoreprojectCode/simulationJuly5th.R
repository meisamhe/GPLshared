...
#================================================================================
T = 150 # total duration of our sample
...
for (t in 1:T){
   for (j in 1:ncat){
      ...
      catIndvlatent[1:nIndv,j,t]=
         gammaIndv[1:nIndv,]*catIndvlatent[j,t]+evIndvcat[1:nIndv,j,t]  
   }
}
...
catEKF= function(ycat,Fcat,pcat,m0cat,C0cat,vcat,wcat,thetacatj) {
   ...
   # Kalman Filtering
   for (t in 1:T){
      gcat = Jccat(mtcat,thetacatj)$newJacob
      ...
      #for (j in 1:pcat){
      result = foreach(j=1:pcat,.combine=rbind) %dopar%{
   ...
   for (t in (T-1):1){
      gcat = Jccat(mcat[,t],thetacatj)$newJacob
...
#       qapp = t(rapp)%x%(Fapp%*%t(Fapp)) + vapp[,]
      ...
      #for (a in 1:napps){
      result = foreach(a=1:napps,.combine=rbind) %dopar%{
...
#          Ctapp[a] = rapp[a] - Aapp%*%qapptemp%*%t(Aapp)
   ...
   tt1app[,T] = mapp[,T]+sqrt(Capp[,T])*as.matrix(rnorm(napps))
   for (t in (T-1):1){
      ...
      tt1app[,t]  = tmapp     + sqrt(Cmapp)*as.matrix(rnorm(napps))                                                                                   
   }
   
   #now ad-hoc treatment of start value
   m0app  = mapp[,1]
   C0app  = Capp[,1]
   tt0app    = m0app     + sqrt(C0app)*as.matrix(rnorm(napps))
...
#                priorLatentVar = priorLatentVar,distortionIndv = distortionIndv)
   ...
   out             =  optim(latentitold,llmnllatent,method="SANN",control=list( fnscale=-1,trace=1,reltol=1e-6,maxit=2000), #hessian = TRUE
...
basslikelihoodCat= function(thetacatjestcur,thetacatjest,tt0cat,tt1cat,wcat,oldcompthetacatj,indthetacatj,curcat,meansuntilcur){ 
   ...
   if (curcat == 1){
      ...
      wcatconditional = wcat[curcat,curcat]
    ...
    #  thetacatjconditional[curcat,] = thetacatjest[curcat,] - delta21%*%meansuntilcur[(1:(curcat-1)),]
      ...
      wcatconditional = wcat[curcat,curcat] - wcat[curcat,(1:(curcat-1))]%*%wcat[(1:(curcat-1)),(1:(curcat-1))]%*%
...
basslikelihoodApp= function(thetaappaestcur,thetaappaest,tt0app,tt1app,wapp,oldcompthetaappa,indthetaappa,curapp,meansuntilapp,tt1cat){ 
   #meansuntilcur is an array but I will use until curcat of it
   napp = dim(tt1app)[1]
   T    = dim(tt1app)[2]
...
wcat = 0.5*diag(c(rep(1,pcat)))
tt0cat = 0.01*matrix(c(rep(1,ncat)),ncol=ncat)
...
wapp = 0.5*diag(rep(1,napps))
tt0app = 0.01*matrix(c(rep(1,napp)),ncol=napp)
...
w0ivcat = ncat
Swivcat = matrix(rep(0,ncat*ncat),ncol=ncat)
...
w0ivapp = napp
Swivapp = matrix(rep(0,napp*napp),ncol=napp)
...
for(iterrep in 1:R){
   ...
   # for category:
   #----------------   
   outcat  =  optim(alphainit,llmnl,method="BFGS",control=list( fnscale=-1,trace=1,reltol=1e-6), 
   ...
   # for apps:
   #----------------   
   outapp  =  optim(betainit,llmnl,method="BFGS",control=list( fnscale=-1,trace=1,reltol=1e-6), 
   ...
   for (i in 1:nlgt) 
   {
      ...
      outcat=optim(betapooledcat,llmnlFract,method="BFGS",control=list( fnscale=-1,trace=0,reltol=1e-4), 
                X=lgtdatac[[i]]$X,y=lgtdatac[[i]]$y,betapooled=betapooledcat,rootH=rootHcat,w=w,wgt=wgtcat)
      outapp=optim(betapooledapp,llmnlFract,method="BFGS",control=list( fnscale=-1,trace=0,reltol=1e-4), 
   ...
   result = foreach(lgt=1:nlgt,.packages=c("MASS","corpcor"),.combine=rbind,.export=c("llmnl","lndMvn")) %dopar%{
      ...
      for (clgt in 1:ncat){
            ...
            vcatmispercept[clgt]    = tempmisperceptcat[clgt,clgt]-
      ...
      metropoutcat=mnlRwMetropOnce(lgtdatac[[lgt]]$y,lgtdatac[[lgt]]$X,oldalphai[lgt,],
                                ...
                                as.vector(indvperceptioncat[lgt,,]),cumjcat[lgt],as.vector(cbind(t(tt0cat),tt1cat[,-T])),
...
#       indvperceptioncat[lgt,,]=matrix(metropoutcat$latentitold,byrow=T,nrow=ncat)
      ...
      for (algt in 1:napps){
         ...
         vappmispercept[algt]    = tempmisperceptapp[algt,algt]-
      ...
      latentapprealTemp = rbind(rep(0,T),cbind(t(tt0app),tt1app[,-T]))
...
#       indvperceptionapp[lgt,1,1:T]=rep(0,T)
   ...
   for (lgt in 1:nlgt){
      ...
      diag(indvperceptionapp[lgt,as.vector(lgtdatac[[lgt]]$y),1:T])=result[[lgt,3]]$indvperceptioncat
      indvperceptionapp[lgt,1,1:T]=rep(0,T)
   }
    result = NULL
...
#    itime=proc.time()[3]   
   ...
   for (clt in 1:ncat){
      ...
      llcatObsEqCur = llcatObsEq - 0.5*sum(diag(chol2inv(chol(vcat[,,clt]))%*%crossprod(errortempcat,errortempcat)))-
...
#     itime=proc.time()[3]  
   result = foreach (clt=1:ncat, .packages=c("numDeriv","foreach","bayesm","corpcor","MASS"), .combine=rbind) %dopar%{
      cat(clt)
      outcatst=optim(thetacatjest[clt,],basslikelihoodCat,method="BFGS",control=list( fnscale=-1,trace=1,reltol=1e-4), hessian=TRUE,
      ...
      if (is.positive.definite(outcatst$hessian )){
         varcattemp  = diag(chol2inv(chol(outcatst$hessian)))
      }else{
         varcattemp  = diag(ginv(make.positive.definite(outcatst$hessian)))
      }
...
#-----------------------------------------------
   ...
   errorwtempcat = t(errorwtempcat)
   
   sigmaiwmucat = chol2inv(chol(Swivcat+crossprod(errorwtempcat,errorwtempcat)))
   # draw from IW(sigmaiwmu,viwmu)
...
#-------------------------------------------------------------------------------------
   ...
   result = foreach (alt=1:napp, .packages=c("numDeriv","foreach","bayesm","corpcor","MASS"), .combine=rbind) %dopar%{
      cat(alt)
      outappst=optim(thetaappaest[alt,],basslikelihoodApp,method="BFGS",control=list( fnscale=-1,trace=1,reltol=1e-4), hessian=TRUE,
...
#-----------------------------------------------
   ...
   errorwtempapp = t(errorwtempapp)
   
   sigmaiwmuapp = chol2inv(chol(Swivapp+crossprod(errorwtempapp,errorwtempapp)))
   # draw from IW(sigmaiwmu,viwmu)
...
#-------------------------------------------------------------------------
   if(((iterrep/100)*100) ==(floor(iterrep/100)*100))
   {ctime=proc.time()[3]
    timetoend=((ctime-itime)/iterrep)*(R+1-iterrep)
