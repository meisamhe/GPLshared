 ...
 *
 * $Id: PreparedStatement.java,v 1.3 2000/07/04 01:10:03 mmatthew Exp $
 *
 ...
 * @version $Id: PreparedStatement.java,v 1.3 2000/07/04 01:10:03 mmatthew Exp $
 */
...
public class PreparedStatement extends org.gjt.mm.mysql.Statement 
{
        ...
        //if (_TemplateStrings == null) {
            ...
            for (i = 0 ; i < _TemplateStrings.length; ++i) {
                if (Encoding == null) {
                    ...
                    try {
                        _TemplateStrings[i] = ((String)V.elementAt(i)).getBytes(Encoding);
                    }
	...
	synchronized (_Conn.getMutex()) {
		...
		if (_has_limit_clause) { 
		    _Results = _Conn.execSQL(null, _max_rows, _SendPacket);
		}
		else {
		    if (_max_rows <= 0) {
	...
	if (X == null) {
	    ...
	    String TimestampString = "'" + _TSDF.format(X) + "'";
	...
	synchronized (_Conn.getMutex()) {
	    ...
	    if (_Conn.useMaxRows()) {
		if (_has_limit_clause) { 
		    RS = _Conn.execSQL(null, _max_rows, _SendPacket);
		}
		else {
		    if (_max_rows <= 0) {
			_Conn.execSQL("SET OPTION SQL_SELECT_LIMIT=" 
    ...
    private final synchronized byte[] streamToBytes(InputStream In) throws java.sql.SQLException
    {
