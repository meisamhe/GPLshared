...
public class DocTermCheckList extends JDialog {
    ...
    private void jbInit(){
        ...
        jSouthPanel.add(jButtonView, new XYConstraints(110, 9, 91, 22));
        jSouthPanel.add(jButtonFinish, new XYConstraints(17, 9, 83, 22));
        list=initCheckList();
...
//    }
    ...
    public void jButtonFinish_mouseClicked(MouseEvent e) {
                ...
                Iterator itDummy=DummyObject.iterator();
                while(itDummy.hasNext()){
    				((ShowDoc)itDummy.next()).dispose();
    			}
    			DummyObject.clear();
    			this.dispose();
        }
    ...
    public void jButtonView_mouseClicked(MouseEvent e) {
    	...
    	if(jTextDoc.getText()!=null){
    		try{
    			Iterator it=DummyObject.iterator();
    			while(it.hasNext()){
    				((ShowDoc)it.next()).dispose();
    			}
    			DummyObject.clear();
...
class DocTermCheckStruct{
        ...
        float weight=0;
        public DocTermCheckStruct(int doc,int term,float weight){
                this.docNumber=doc;
