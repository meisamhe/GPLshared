...
namespace RedBlackTree
{
	class treenode
	{
		...
		public treenode(int data ) : this (false,data,0, null, null, null){}
		public treenode(bool color, int data, int size, treenode left, treenode right,
			treenode parent)
		{
            this.size = size;
			this.color = color;
	...
	class RedBlackTree
	{
		...
		public void RightRotate(treenode temp)
		{
			...
			temp.Parent = prev;
			prev.Size =temp.Size;
			temp.Size = temp.Right.Size + temp.Leftt.Size + 1;
		}
		...
		public treenode OSSelect( treenode temp, int numerator)
		{
			int LChildNumber = temp.Left.Size + 1;
			if ( numerator == LChildNumber )
				return temp;
			else if ( numerator < LChildNumber )
				return OSSelect (temp.Left , numerator );
			else return OSSelect(temp.Right , numerator - LChildNumber );
		}

		public int OSRank ( treenode temp )
		{
			int LChildNumber = temp.Left.Size + 1;
			treenode cur = temp;
			while ( cur != root )
			{
				if ( cur == cur.Parent.Right)
					LChildNumber += cur.Parent.Left.Size + 1;
				cur = cur.Parent;
			}
			return LChildNumber;
		}
