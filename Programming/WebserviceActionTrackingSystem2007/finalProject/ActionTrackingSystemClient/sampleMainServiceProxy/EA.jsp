...
<DIV id=mainContentBorder align=center  style="width:1000"">
...
href="ActionTrackingSystem.jsp"><img src="logout.gif"></A> </TD>
  ...
  <DIV 
style="PADDING-RIGHT: 200px; PADDING-LEFT: 0px; PADDING-BOTTOM: 0px; MARGIN: 0px; PADDING-TOP: 0px; HEIGHT: 46px; width:1200px">
	<IMG 
style="BACKGROUND-COLOR: #d5ae65" height=5 alt="" 
src="ActionTrackingSystem_files/spacer.gif" width=1013 border=0></DIV>
<DIV id=mainContent style="width:1000">
  <DIV id=mainContentMiddleBorder style="width:1000">
<DIV id=mainBody>
<%try{%>
	  ...
	  <table width="815" height="87" border=1 cellpadding=3 cellspacing=0 class="sortable"  id=sections >
        <tbody>
          <tr bgcolor="#999999" class="affHeadline">
            ...
            <td width="93"  align="LEFT" bgcolor="#FFFF99"><div align="center"><span class="style4">Deadline</span></div></td>
            ...
            <td width="67"  align="LEFT" bgcolor="#FFFF99" ><div align="center"><span class="style4">AssignedTo</span></div></td>
            ...
            <td width="37"  align="LEFT" bgcolor="#FFFF99" style="text-align: center">Report</td>
...
/*if (type_159id.compareTo("manager")==0){
 ...
 String type_101id= (String) session.getAttribute("TYPE");
        ...
        String user_103id=(String)  session .getAttribute("USER");
        java.lang.String user_103idTemp  = user_103id;
        uncompletedActivityUntilDate =  sampleMainServiceProxyid.uncompletedActivityUntilDateOfsubsidary(date_102idTemp,user_103idTemp);
%>
          <%
...
java.lang.String typename292 = activity.getName();
        ...
        %>
		 <td bgcolor="#FFFFFF" class="border" ><img src="attention_red.png" hight=20 width=20></td>
            <td bgcolor="#CCFFFF" class="border" ><p align="center" class="style6 style4 style4 style5">
            <FORM name=form<%=i%> 
...
%>
       ...
       <tr>
       <td>
        <input type=text name=deadLine<%=i%>  size=6 value="<%=s%>">
       </td>
       <td>
       	<a href="javascript:void(0)" onClick="showCal('calDate<%=i%>'); return false;" hidefocus><img name="popcal" width="18" height="20" len="absmiddle" src="calender.jpg" border="0" alt="Select Date"></a>
                <%}%>
       </td>
       </tr>
       </table>
...
%>
				<%
				com.actionTracking.Person p=activity.getEmployee();
				%>
                <%//=activity.getEmployeeID()
%>				<input type=text style="visibility:hidden" name=assignedPerson<%=i%> value="<%=p.getUser()+"_"+p.getType()%>">
				<select name=assignedTo<%=i%>>
         		   <option value="<%=p.getUser()+"_"+p.getType()%>" width="53" size="2"><%=p.getName()+"_"+p.getFamily()+"("+p.getDepartment()+")"%></option><%
				   	String type_130id= "manager";
  			     	java.lang.String type_130idTemp  = type_130id;%>
					<%//=type_130id%>
					<%
   			   	com.actionTracking.Person[] getPersonel383mtemp = sampleMainServiceProxyid.getPersonel(type_130idTemp);
					java.util.List personList;
if(getPersonel383mtemp == null){
	personList=null;
}else{
    personList= java.util.Arrays.asList(getPersonel383mtemp);
}
if (personList!=null){

for(int i1=0; i1<personList.size();i1++){
com.actionTracking.Person tempPerson=getPersonel383mtemp[i1];
%>
  <option value="<%=tempPerson.getUser()+"_"+tempPerson.getType()%>"><%=tempPerson.getName()+"_"+tempPerson.getFamily()+"("+tempPerson.getDepartment()+")"%></option>
<%
}
}
type_130idTemp= "employee";
getPersonel383mtemp = sampleMainServiceProxyid.getPersonel(type_130idTemp);
if(getPersonel383mtemp == null){
	personList=null;
}else{
    personList= java.util.Arrays.asList(getPersonel383mtemp);
...
<%
				...
				<%//=p.getName()+"_"+p.getFamily()+"("+p.getDepartment()+")"%>
            </div></td>
            <td  >
			  <input type=text style="visibility:hidden" size=1 value="updateActivityManager" name="requestType"> 
...
java.lang.String typedescription298 = activity.getDescription();
        ...
        %>
                <div align="center">
                  <form method="post" action="Description.jsp" target="_blank">
                    <div id="div" style="display: none;">
                      <input type="text" name=description value="<%=tempResultdescription298%>" size=0>
                    <input type="text" name=person size=1 style="visibility:hidden" value="<%=activity.getEmployee().getUser()+"_"+activity.getEmployee().getType()%>">
                    <input type="text" name=activityID size=1 style="visibility:hidden" value="<%=activity.getActivityId()%>"></div>
            ...
            <form method="post" action="Report.jsp" target="_blank">
              &nbsp;<input type="text" name=person size=1 style="visibility:hidden" value="<%=activity.getEmployee().getUser()+"_"+activity.getEmployee().getType()%>"><!-- type is also included-->
              <input type="image" name=I1 width="26" height="24"  src="report.jpg" border="0" >
              <input type="text" name=activityID size=1 style="visibility:hidden" value="<%=activity.getActivityId()%>">
              </form>
              
              
            </div></td>
            <td  > <div align="center">
             <form method="post" action="Resource.jsp" target="_blank">
              &nbsp;<input type="text" name=person size=1 style="visibility:hidden" value="<%=activity.getEmployee().getUser()+"_"+activity.getEmployee().getType()%>"><!-- type is also included-->
              <input type="image" name=I width="26" height="24"  src="resource.jpg" border="0" >
              <input type="text" name=activityID size=1 style="visibility:hidden" value="<%=activity.getActivityId()%>">
              </form>
...
<DIV id=joinandFooter style="width:1000"><BR style="CLEAR: both; LINE-HEIGHT: 0; HEIGHT: 0px">
<p></p></DIV><!-- END OF JOIN ActionTrackingSystem EMAIL LIST --><p></p></DIV><p></p></DIV>
