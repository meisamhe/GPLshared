...
public class excelParser implements Parser {
    ...
    void readDB(){
       ...
       HSSFWorkbook wb=null;
       try {
           fs = new POIFSFileSystem(new FileInputStream("words.xls"));
           wb = new HSSFWorkbook(fs);
       ...
       for ( int i =0;;i++){
           row = sheet.getRow(i);
           if (row == null){
               System.out.println("cellIsNull");
               break;
           }
           cell = row.getCell((short) 0);
           cell = row.getCell((short) 0);
           String Word = this.getCellValue(cell);
           cell = row.getCell((short) 1);
           String Meaning=this.getCellValue(cell);
           System.out.println(Word+" "+Meaning);
           msaccessdb.insert(Word,Meaning);
           //   cell.setCellType(HSSFCell.CELL_TYPE_STRING);
         //  cell.setCellValue("Hi Meisam");
       }
       msaccessdb.close();
