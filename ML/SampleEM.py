...
def expectation_maximization(t, nbclusters=2, nbiter=3, normalize=False,\
    ...
    -> Draw nbclusters sets of (mu, sigma, P_{#cluster}) at random (Gaussian
       Mixture) [P(Cluster=0) = P_0 = (1/n).sum_{obs} P(Cluster=0|obs)]
    ...
    [M] * P(Cluster=0)^{t+1} = (1/n).sum_{obs} P(Cluster=0|obs)
        * mu^{t+1}_0 = sum_{obs} obs.P(Cluster=0|obs) / P_0
        * sigma^{t+1}_0 = sum_{obs} P(Cluster=0|obs)(obs-mu^{t+1}_0)^2 / P_0
    -> Compute E_t=sum_{obs} log(P(obs)^t)
       Repeat Steps 2 and 3 until |E_t - E_{t-1}| < epsilon
    """
    def pnorm(x, m, s):
        """
        ...
        """
        xmt = np.matrix(x).transpose()-np.matrix(m).transpose()
        for i in xrange(len(s)):
            if s[i,i] <= sys.float_info[3]: # min float
                s[i,i] = sys.float_info[3]
        sinv = np.linalg.inv(s)
        xm = np.matrix(x)-np.matrix(m)
        ...
        while (abs(log_estimate - old_log_estimate) > epsilon\
            ...
            for o in xrange(nbobs):
                for c in xrange(nbclusters):
                    # Px[o,c] = P(x|c)
                  #  print 'the x vector is %s'%t[o,:]
                  #  print 'the mu vector is %s'%params[c]['mu']
                  #  print 'the difference between them is:%s'%(np.matrix(t[o,:])-np.matrix(params[c]['mu']))
                    Px[o,c] = pnorm(t[o,:],\
            ...
            for c in xrange(nbclusters):
                ...
                for o in xrange(nbobs):
                    m = np.matrix(t[o,:])*np.matrix(Pclust[o,c]) + np.matrix(m)
