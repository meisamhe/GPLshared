...
for i = 1:ndraw
   ...
   alpha     = 0.5;
   accptnrnd = 0.8;
   [x,fval,exitflag,output,grad,hessian]=fminunc('HorskeySimonMethodM0',X0,options,yc,ppparam,npparam,qpparam,ttind1p,ttr1,Lpriorp,[m0p mp(:,1:pT-1)]',wp);
   var        = inv(hessian);
   if (hessian <= 0)
       var = 1e-6;
   end;
...
%   pc = pcorigin;
   ...
   while ((alpha < accptnrnd)) %&& (swfac == 1) )
       ...
       postPNew  = -HorskeySimonMethodM0(M0paramNew,yc,ppparam,npparam,qpparam,ttind1p,ttr1,Lpriorp,[m0p mp(:,1:pT-1)]',wp); % log of new posterior
       postPOld  = -HorskeySimonMethodM0(M0param,yc,ppparam,npparam,qpparam,ttind1p,ttr1,Lpriorp,[m0p mp(:,1:pT-1)]',wp); % log of old posterior
...
%    end;
  ...
  % f = -qpparam/x*(2*ttind1p.*m-m.^2) + (1+qpparam-ppparam).*ttind1p-(yc(1:(size(yc,1)-1),:)*npparam).*ttind1p+ppparam*x + yc(1:(size(yc,1)-1),:)*npparam*x;
   ttindtemp = [(-2*[m0p mp(:,1:pT-1)]'.*ttind1p+[m0p mp(:,1:pT-1)]'.^2)/M0param+ttind1p M0param-ttind1p yc(1:(size(yc,1)-1),:).*repmat((M0param-ttind1p),[1 size(yc,2)])];
   ttrtemp = ttr1;
   ttr1    = ttr1 - ttind1p;
   ttind1p = ttindtemp;
