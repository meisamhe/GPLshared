...
%============================ Data ======================================
%==========================================================================
path ='C:\Users\MeisamHe\Desktop\Projects\OpenPlatformFirefoxAddon\Data\';
...
% read AMO Data
[num,txt,raw] = xlsread(strcat(path,'AMODATA\AMOCleaned.xlsx'),1);
totalNomQueue = num(:,11)/100; %total nomination in queue (updated to daily entrants)
...
pqM0betaNewtemp  = zeros(parallelsize,kp);
 ...
 %-----------------------------------------------------
 TNC=totalNomContrib-mean(totalNomContrib);
 TNC2=zeros(pT,1);
 v0iv =5;
 Sviv = [0.1 -0.05;-0.05 0.1];
 sigmaiv = [0.2 0; 0 0.5]; 
 ivp_ = zeros(5,(ndraw-ndraw0)/jumps); %first four elements of covar matrix, and fifth element is corr(e(1t),e(25))
 tt1mu    =  15*ones(1,pT); 
 tt0mu=0;
 gamma1=0;
 gamma2=0;
 bmu_ =  zeros(pT*1,(ndraw-ndraw0)/jumps);
 b0mu_ = zeros(1,(ndraw-ndraw0)/jumps);
 Fmu=ones(pT+1,1);
 mmu=zeros(1,pT);
 Cmu = repmat(eye(1),[1 1 pT]);
 m0mu = ones(1,1);
 C0mu = repmat(eye(1),[1 1]);
 GTmu = [0.1 0.8 0.2];
 wmu = 0.8;
 smuInv = 1.5e-7*eye(size(GTmu,1));
 meanmu    =  zeros(size(GTmu,1),1);
 uTmu = GTmu(1) + GTmu(3)*yc(1:(size(yc,1)-1),4);
 cmu_ = zeros(size(GTmu,1)+2,(ndraw-ndraw0)/jumps); %gamma1,gamma2,w2,v|w1
...
%     uT=uTp(1:pT);
    ...
    [mp(:,1:pT),Cp(:,:,1:pT),m0p(:,:),C0p,tt1p(1:pT),tt0p]     =     BEKFPMOnlyPlatformCompetitorIV(yp(1:pT),F,pp,m0p(:,:),C0p(:,:),vp,wp,GTp(:,:),uTp(1:pT),yc,caddon);   %,a,b,alpha
   ...
   % npparam   = btp(3:(2+size(yc,2)));
    ...
    %---------------------------------------------------------------------------------
    TNC2= ttr1-...
        (-qpparam./(M0param+eta*caddon(1:(size(yc,1)-1),:)).*(2*ttind1p.*[m0p mp(:,1:pT-1)]'-[m0p mp(:,1:pT-1)]'.^2) + (1+qpparam-ppparam).*ttind1p-(yc(1:(size(yc,1)-1),1:2)*npparam(:,1:2)'...
        +yc(1:(size(yc,1)-1),4)*npparam(:,4)').*ttind1p+ppparam*(M0param+eta*caddon(1:(size(yc,1)-1),:)) + (yc(1:(size(yc,1)-1),1:2)*npparam(:,1:2)'...
        +yc(1:(size(yc,1)-1),4)*npparam(:,4)').*(M0param+eta*caddon(1:(size(yc,1)-1),:)));
    
    Fmu=[ones(pT,1) npparam(:,3)*(M0param+eta*caddon(1:(size(yc,1)-1),:)-ttind1p)];
    ymu=[TNC TNC2];
...
%     GT = GTmu;
    ...
    [mmu,Cmu,m0mu,C0mu,tt1mu] = kalffbsMeasurement(ymu,Fmu,1,m0mu,C0mu,sigmaiv,wmu,GTmu,uTmu);
    
    %update the data
    yc(:,3) = tt1mu; %replace for the next iteration of EKF
    
    % estimate non-state parameter of System equation of AMO
     X = [ones(pT,1) tt1mu(1:pT)' yc(1:(size(yc,1)-1),4)];
     Y = tt1mu(2:(pT+1))';
     S1   =  (((X'*X)/wmu)+smuInv)\speye(size((((X'*X)/wmu)+smuInv))); 
     b1   =  S1*(((X'*Y)/wmu)+smuInv*meanmu); %btempp(j,1)
     gammamu   =  b1 + chol(S1)'*randn(length(b1),1);
     GTmu=gammamu;
     uTmu = GTmu(1) + GTmu(3)*yc(1:(size(yc,1)-1),4);
     
     %estimate variance wmu (System equation of AMO)
     wa1  = (wa0+pT)/2;
     wv12 = (wv02+(Y-X*gammamu)'*(Y-X*gammamu))/2; %btempp(j,1) btempp(j,1)
     wmu   =  1/gamrnd(wa1,1/wv12);
    
    
    %simulate w ~ IG (System Equation)
%     f = -qpparam./(M0param+eta*caddon(1:(size(yc,1)-1),:)).*(2*ttind1p.*[m0p mp(:,1:pT-1)]'-[m0p mp(:,1:pT-1)]'.^2) + (1+qpparam-ppparam).*ttind1p-(yc(1:(size(yc,1)-1),:)*npparam').*ttind1p+ppparam*(M0param+eta*caddon(1:(size(yc,1)-1),:)) + (yc(1:(size(yc,1)-1),:)*npparam').*(M0param+eta*caddon(1:(size(yc,1)-1),:));
    f = npparam(:,3)*(M0param+eta*caddon(1:(size(yc,1)-1),:)-ttind1p).*tt1mu(1:pT)';
    ewv = TNC2-f;
    %plot(1:pT,TNC2,1:pT,f)
    evmu = TNC-tt1mu(2:(pT+1))';
	...
	sigmaiv = inv(sigmaivinv);
    
    wp = sigmaiv(2,2)-sigmaiv(2,1)/sigmaiv(1,1)*sigmaiv(1,2);
    vmu = sigmaiv(1,1)-sigmaiv(1,2)/sigmaiv(2,2)*sigmaiv(2,1);
    
    disp('vp and wp for diffusion equation is:')
    disp([vp; wp])
    disp('vmu and wmu for measurement equation is:')
    disp([vmu; wmu])
    
    corrmuobsttst = corr(ewv,evmu);
    %ecmnobj([evmu ewv],[0 0],sigmaiv)
    ...
    if (sw==1)     
       ...
       % LL (jp)      = LL(jp)- 0.5*(evv'*evv)/vp -   0.5*pT*pp*log(2*pi*vp);
        ...
        % IV portion added
        %------------------------
        ivp_(:,jp)   = [sigmaiv(:);corrmuobsttst];
        bmu_(:,jp)    = tt1mu(2:(pT+1))';
        b0mu_(:,jp)   = tt1mu(1);
        cmu_ (:,jp)   = [GTmu;vmu;wmu];
        %LL (jp)      = LL(jp)- 0.5*([evmu ewv]'*[evmu ewv]*inv(sigmaiv)) -   0.5*2*pT*1*log(2*pi*det(sigmaiv));
        for itemp=1:size(evmu,1)
             LL(jp) = LL(jp)- 0.5*([evmu(itemp) ewv(itemp)]*inv(sigmaiv)*[evmu(itemp) ewv(itemp)]') -   0.5*2*1*log(2*pi*det(sigmaiv));
        end;
        LL (jp)      = LL(jp)- 0.5*((Y-X*b1)'*(Y-X*b1))/wmu -  0.5*pT*pp*log(2*pi*wmu);
...
%------------------------------------------------------------------------------------
%calculate iv portion of likelihood of platform (joint)
%------------------------------------------------------------------------------------
evmu = TNC- mean(bmu_,2);
sigmaiv=reshape(mean(ivp_,2),[2,2]);
TNC2= ttr1-...
    (-qpparamLL./(M0paramLL+etaLL*caddon(1:(size(yc,1)-1),:)).*(2*ttind1pLL.*[m0pLL mpLL(:,1:pT-1)]'-[m0pLL mpLL(:,1:pT-1)]'.^2) + (1+qpparamLL-ppparamLL)...
    .*ttind1pLL-(yc(1:(size(yc,1)-1),1:2)*npparamLL(:,1:2)'...
    +yc(1:(size(yc,1)-1),4)*npparamLL(:,4)').*ttind1pLL+ppparamLL*(M0paramLL+etaLL*caddon(1:(size(yc,1)-1),:)) + (yc(1:(size(yc,1)-1),1:2)*npparamLL(:,1:2)'...
    +yc(1:(size(yc,1)-1),4)*npparamLL(:,4)').*(M0paramLL+etaLL*caddon(1:(size(yc,1)-1),:)));
tt1mu = mean(bmu_,2);
f = npparamLL(:,3)*(M0paramLL+etaLL*caddon(1:(size(yc,1)-1),:)-ttind1pLL).*tt1mu(1:pT)';
ewv = TNC2-f;
LLmean      = LLmean- 0.5*([evmu ewv]'*[evmu ewv]*inv(sigmaiv)) -   0.5*2*pT*1*log(2*pi*det(sigmaiv));
X = [ones(pT,1) tt1mu(1:pT)'];
Y = tt1mu(2:(pT+1))';
coeffmu=mean(cmu_,2);
b1 = coeffmu(1:2);
wmu = coeffmu(3);
LLmean      = LLmean- 0.5*((Y-X*b1)'*(Y-X*b1))/wmu -  0.5*pT*pp*log(2*pi*wmu);
...
%-------------------------------------
ivpmean_ = mean(ivp_,2);
disp('parameter estiamte for [s11 s21 s12 s22 corr] are (posterior):');
disp(ivpmean_');
...
[n1,n2]=size(ivp_);
 hpdismu=zeros(n1,2);
  for ii=1:n1
     hpdismu(ii,1:2) = hpdi(ivp_(ii,:)',.95);
  end
  hpdismu;
  
%-----------------------------------------------------------------------------------------
%test convergence and parameter heterogeneity
%---------------------------------------------------------------------------------------------
plot(c_temp(1,:,3));
