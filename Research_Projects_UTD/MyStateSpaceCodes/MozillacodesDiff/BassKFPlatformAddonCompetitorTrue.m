...
% postPOld = cell([1 n]);
 ...
 pcaddon =0.5e-3; %tunning parameter
...
for i = 1:ndraw 
  ...
  % platform EKF calculation
   ...
   alphas     = 0.5;
   accptnrnd = 0.8;
   [M0temp,fval,exitflag,output,grad,hessian]=fminunc('HorskeySimonMethodM0EKF',X0,options,yc,ppparam,npparam,qpparam,ttind1p,ttr1,Lpriorp);
   wp        = inv(hessian);
   if (hessian <= 0)
       wp = 1e-6;
   end;
   ...
   spmd
      ...
      while ~done
       ...
       postPNew  = -HorskeySimonMethodM0EKF(M0paramNew,yc,ppparam,npparam,qpparam,ttind1p,ttr1,Lpriorp); % log of new posterior
       postPOld  = -HorskeySimonMethodM0EKF(M0param,yc,ppparam,npparam,qpparam,ttind1p,ttr1,Lpriorp); % log of old posterior
