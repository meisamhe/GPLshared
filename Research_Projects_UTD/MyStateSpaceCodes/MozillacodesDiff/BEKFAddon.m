...
function [m,C,m0,C0,tt1,tt0,Y1,MAD,MSE] = BEKFAddon(y,F,p,m0,C0,v,w,GT,uT,j,pT,mp,m0p,aipi,tt1p,tt0p,olst) %,b1,b2,alpha
...
% Modified: Meisma Hejazinia ... Dec 2013
 ...
 for t      =   1:T
         ...
         a      =   fmaddon(d,mt,t,T,pT,tt1p,tt0p,olst) + uT(t,:); %alpha,
         forecast = ha(a); 
       ...
       %  size(uT)
        % g      =   Jmaddon(d,mt,t,T,pT,mp,m0p,aipi,tt1p,tt0p,olst); %alpha,  
         g      = Jmaddon(d,mt,t,T,pT,tt1p,tt0p,olst);
         r      =   g(1,1)*Ct*g(1,1)' + w;
 ...
 for t    =   (T-1):-1:(1)
  ...
  %   g    =   Jmaddon(d,m(:,t),t+1,T,pT,mp,m0p,aipi,tt1p,tt0p,olst); %alpha,
     g      = Jmaddon(d,m(:,t),t+1,T,pT,tt1p,tt0p,olst);
     r    =   g(1,1)*C(:,:,t)*g(1,1)' + w;
     b    =  C(:,:,t)*g(1,1)'/r;
     u    =  C(:,:,t)*g(1,1)';
     ...
     a    =   fmaddon(d,m(:,t),t+1,T,pT,tt1p,tt0p,olst)+ uT(t+1,:); %alpha,
     tm   =   m(:,t) + b*(tt1(:,t+1)-a); 
     if min(diag(Cm)) < 0 
