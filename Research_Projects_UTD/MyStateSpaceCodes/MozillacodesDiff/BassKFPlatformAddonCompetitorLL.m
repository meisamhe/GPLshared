...
% using Simon and Horskey for competition
% using Schmitten and Mahajan 1982 Maximum Likelihood
% (Over full Data)
% estimate
   ...
   %simulate GT = d, beta (System Equation)
    ...
    parfor j=1:n;
        disp(j)
           ...
           alphasaddon     = 0.5;
           accptnrnd = 0.8;
           [betatemp,fval,exitflag,output,grad,hessian]=fminunc('SchmittleinMahajan1982',X0,options,tt0{j},tt1{j},T(j),pT,tt1p); % implicit prior
           wj        = (hessian)\speye(size(hessian)); %    inv(hessian);
           if (hessian <= 0)
               wj = 1e-6;
           end;
           tempj=1;
          while (alphasaddon < accptnrnd)
                % each lab does some computation in parallel
               tempj=tempj+1
               disp(tempj)
               wmj =pcc*wj;
               wmj =max(wmj,1e-50);
               wmj =min(wmj,1e50); % to make sure we can have cholesky
%                if (wmj==0)
%                    wmj = 1e-6;
%                end;
               %betatempNew   =  betatemp + chol(wmj)'*randn(length(betatemp),1);
               postPNew =NaN;
               while (isnan(postPNew))
                   betatempNew   =  betatemp + diag(wmj)'*randn(length(betatemp),1);
                   postPNew  = -SchmittleinMahajan1982(betatempNew,tt0{j},tt1{j},T(j),pT,tt1p); % log of new posterior
               end;
               postPOld  = -SchmittleinMahajan1982(btemp(:,j),tt0{j},tt1{j},T(j),pT,tt1p);  % log of old posterior
               
               alphasaddon     = postPNew - postPOld
               accptnrnd = log(rand(1,1))
               postPNewtemp =postPNew;
               if tempj > 1e6
                   betatempNew = betatemp;
                   postPNewtemp = postPOld;
                   break;
               end;
               % M0paramNew = M0param + pc*randn(length(M0Param),1); % for random walk
          end
           %while ((alpha < accptnrnd)) %&& (swfac == 1) )    end;
           cumja(j)= cumja(j)+tempj;
           % check acceptance rate and modify it
           accepRate=i/cumja(j) % because i out of all cumj iterations is accepted until now
           disp(accepRate)
           ...
           bt = btemp(:,j);
            ...
            GT   (:,:,j)       =   diag([-bt(2)/bt(3) (1-bt(1)+bt(2))]); %  bt(1:2,1));

            uT{j} =   bt(1)*bt(3)*x{j}.*[tt0p tt1p(pT-T(j)+1:pT-1)];%bt(3,1)*x{j}.*[tt0p tt1p(pT-T(j)+1:pT-1)]; %uT includes relevant market size in it
            piai{j} = bt(1)*bt(3);%bt(3,1); % to use in the hap function
        
        %simulate w ~ IG (System Equation)
         ttr1=tt1{j}';
         ttind1= addonttindepbuilder(tt0{j},tt1{j},T(j),x{j},tt1p,pT) ; % drop last value
         wa1  = (wa0+T(j))/2;
         wv12 = (wv02+(ttr1-ttind1(1:T(j),1:2)*[-bt(2)/bt(3);(1-bt(1)+bt(2))]-uT{j}')'*(ttr1-ttind1(1:T(j),1:2)*[-bt(2)/bt(3);(1-bt(1)+bt(2))]-uT{j}'))/2;
         w (j)  =  1/gamrnd(wa1,1/wv12);
        
        if (sw==1);
           c_{j}          =     [bt;v(j);w(j)];
           LLtemp (j)      =  LLtemp (j)+ postPNewtemp;
           
           %(ttr1-ttind1(1:T(j),1:2)*bt(1:2,1)-uT{j}')'*(ttr1-ttind1(1:T(j),1:2)*bt(1:2,1)-uT{j}')/w (j).^2 - ...
           % 0.5*T(j)*p*log(2*pi*w(j).^2);
           errw_{j}  =  errw_{j}+ postPNewtemp;
           %+(ttr1-ttind1(1:T(j),1:2)*bt(1:2,1)-uT{j}');
