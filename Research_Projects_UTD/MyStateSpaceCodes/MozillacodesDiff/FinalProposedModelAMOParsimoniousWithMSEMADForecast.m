...
%==============================================================================================================
% Significance and standard deviation for cross sections (Across Add-ons)
...
%=========================================================================================
% Calculating MAD, MSE, Forecast and curves
%=============================================================================================

% -------------------------------------------------------------------------
% One step ahead forecast
%--------------------------------------------------------------------------
% 1-step ahead forecast for add-ons
%--------------------------------------------------------------------------
...
for numrtr=1:4;
    ...
    tx = 1:T(1,currplugin); 
    subplot(2,2,numrtr);
    plot(tx,Y1{currplugin}, '-.r' ,tx,y{currplugin},'--k' );
...
%--------------------------------------------------------------------------
% 1 step ahead forecast for platform
%--------------------------------------------------------------------------
% 1-step ahead forecast
for i=1:p
tx = 1:pT; plot(tx,Y1p, '-.r' ,tx,yp,'--k' );
title('Step Ahead Forecast'); xlabel('Time');
ylabel('y(t)'); legend('Predicted','Actual');
title('Mozilla Firefox Platform'); xlabel('Days');ylabel('Usage'); 
end;
set(legend('One step ahead forecast','Actual'),'FontAngle','italic','TextColor',[.3,.2,.1],'Location','SouthOutside','Orientation','horizontal');
figureHandle = gcf;
%# make all text in the figure to size 14 and bold
...
MSEM=MSEp/pT
 ...
 %====================================================================================
  ...
  VariableName_ = ['             Relevance           ';
                   'Unobserved External Market Forces';
                   '        Add-on New Version       ';
                   '  Firefox Platform New Version   ';
                   'Unobserved Internal Market Forces';
                   '  Add-on Rating Daily Variance   ';
                   '  Add-on Daily number of Users   ';
                   '     Add-on Rating Average       ';
                   '              Churn              '];
for numrtr=1:9;
    subplot(3,3,numrtr);
    hist(g_(numrtr,:),20);
...
%---------------------------------------------------------------------------------------------
%       Evolution of Each Variable Basic Stat Diagram
%---------------------------------------------------------------------------------------------
tx = (1:pT)';
plot(tx,yp,'-.b',tx,[yie ych],'--r',tx,caddon(2:pT+1,:)/100,'g',tx,yc(2:pT+1,3),'--c',tx,yc(2:pT+1,4),'-.m',tx,ffxvrsn,'--k');
title('Mozilla Firefox-Platform Variable Evolutions'); xlabel('Days');
ylabel('Quantity');
set(legend('     Firefox (10M)  ',...
           '       IE (10M)     ',...
           ' Google Chrome (10M)',...
           'Add-on Created (10K)',...
           '  AMO contrib. (100)',...
           '    AMO Queue (100) ',...
           ' FF New Version'),...
           'FontAngle','italic','TextColor',[.3,.2,.1],'Location','SouthOutside','Orientation','horizontal');
figureHandle = gcf;
%# make all text in the figure to size 14 and bold
