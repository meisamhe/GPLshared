...
%============================ Data ======================================
%==========================================================================
path ='H:\FirefoxProjectFinalRun\firefoxprojectfiles\DataFinal\DataFinal\Projects\FirefoxAddon(VeryLowPriority)\';
...
format long
scale = 10^7;
[num,txt,raw] = xlsread(strcat(path,'DiffData\platformdata.xlsx'),1);
...
% read AMO Data
[num,txt,raw] = xlsread('H:\FirefoxProjectFinalRunserver2\firefoxprojectfiles\DataFinal\DataFinal\Projects\FirefoxAddon(VeryLowPriority)\AMODATA\AMOCleaned.xlsx',1);
...
[numCatOL,txtCatOL,rawCatOL] = xlsread(strcat(path,'DiffData\AddonCategoryLevelOLVar.xlsx'),1);
[numCatVAR,txtCatVAR,rawCatVAR] = xlsread(strcat(path,'DiffData\AddonCategoryLevelOLVar.xlsx'),2);
parfor i=1:n;
    i
    [numaddon,txt,raw] = xlsread(strcat(path,'AddonData\DailyOf100AddOn (Autosaved).xlsx'),pluginlists(1,i));
...
CSINT =zeros(n,csk);
 ...
 %[7,28,29,33,34,35,36];
 [num,txt,raw] = xlsread(strcat(path,'DiffData\AnalysisOFStepWiseRegression.xlsx'),2); 
...
% pitmes = [3,4,5,6,7,11,12];
 ...
 %[7,24,30,34,35,36];
 [num,txt,raw] = xlsread(strcat(path,'DiffData\AnalysisOFStepWiseRegression.xlsx'),3); 
 % for p0
 p0total = num(:,p0itmes);
 ...
 %[11,27,29,30,31];
 [num,txt,raw] = xlsread(strcat(path,'DiffData\AnalysisOFStepWiseRegression.xlsx'),4); 
 % for q0
 q0total = num(:,q0items);
 ...
 %[4,33,34,35,36];
 [num,txt,raw] = xlsread(strcat(path,'DiffData\AnalysisOFStepWiseRegression.xlsx'),5); 
...
vLL      =  ones(1,n)*0.2;      wLL = ones(1,n)*0.5; 
 ...
 %---------------------------------------------------------------------------------
 %IV prior (instrumental variable prior)
 %---------------------------------------------------------------------------------
 NV1    = cell([1 n]);                      % for first observation equation, new version of add-on
 NV2    = cell([1 n]);                      % for second observation equation, new version of platform
 NV3    = cell([1 n]);                      % for third observation equation, state equation of diffusion
 sigmaiv= cell([1 n]);                      % for the var-cover matrix
 ivp_   = cell([1 n]);                      % to save the covariance matrix and correlation elements
 tt1mu  = cell([1 n]);                      % for the latent measure for the (1)version of platform (2) version of add-on
 tt0mu  = cell([1 n]); 
 GTmu   = cell([1 n]);
 nGTmu  = 2;                               % only two coefficients now
 wmu    = cell([1 n]);                      % for the variance of state equation for the filtered version
 v0iv =5;                              % define same prior for all
 Sviv = [0.1 -0.05 -0.05;-0.05 0.1 -0.05;-0.05 -0.05 0.1];        % define same prior for variance of all
 smuInv = 1.5e-7*eye(nGTmu);
 meanmu    = zeros(nGTmu,1);
 uTmu      = cell([1 n]);
 Fmu       = cell([1 n]);
 % latent state mean and variance
 mmu  = cell([1 n]);
 Cmu  = cell([1 n]);
 m0mu = cell([1 n]);
 C0mu = cell([1 n]);
 ...
 parfor i=1:n; %for all add-ons defining control function related parameters
     i
     NV1{i}     = phetrgn{i}(:,2);           % for new version of add-on
     NV2{i}     = phetrgn{i}(:,3);           % for new version of platform
     NV3{i}     = zeros(T(i),1);               % for state equation of add-on diffusion                       
     sigmaiv{i} = [0.2 0 0;0 0.2 0;0 0 0.5]; % 
     ivp_{i}    = zeros(6+3,(ndraw-ndraw0)/jumps); %first sixth elements of  triangular elements of covar matrix, and 
                                                %tenth and eleventh element
                                                %is corr(e(1t),e(2t)) for
                                                %version's and state corr
 end;
  parfor i=1:n; %for all add-ons defining control function related parameters
      i
     tt1mu{i}    =  15*ones(2,T(i)); 
     tt0mu=[0 0];
     GTmu{i} = [0.1 0.8; 0.1 0.8];          % for the state equation coefficient
     wmu{i}  = diag([0.8; 0.8]);
     uTmu{i} = [0.1; 0.1];                % only intercept now     
     Fmu{i}  =zeros(T(i),2,3);             %  the first two columns are one
  end;
  parfor i=1:n; %for all add-ons defining control function related parameters 
      i
     % latent state mean and variance
     mmu{i}    = zeros(2,T(i));
     Cmu{i}    = repmat(eye(1),[2 2 T(i)]);
     m0mu{i}   = ones(2,1);
     C0mu{i}   = repmat(eye(2),[1 1]);
  end;
  w1 = cell([1 n]);
  w2 = cell([1 n]);
 for i=1:n; %for all add-ons defining control function related parameters 
      i   
     bmu_{i} =  zeros(T(i)*1,(ndraw-ndraw0)/jumps);
     b0mu_{i} = zeros(1,(ndraw-ndraw0)/jumps);
     cmu_{i} = zeros(nGTmu*nGTmu+2+3+1,(ndraw-ndraw0)/jumps); %gamma1,gamma2,w2,v|w1, one correlation item
     w1{i}      = 0.5;
     w2{i}      = 0.5;
 end;
 Y1 = cell([1 n]);
...
%         disp([alphaMaddon betaaddon' gammaaddon']) %pparamaddon qparamaddon
        ...
        %---------------------------------------------------------------------------------
        %                   Endogeneity Portion (Control Function approach)
        %---------------------------------------------------------------------------------
        pqalphatempaddon = [-gammaaddon*(1-deltaaddon)/alphaMaddon;1-deltaaddon;-betaaddon(1);alphaMaddon.*betaaddon(1);(1-deltaaddon)*gammaaddon];
        IVttind1addon= IVaddonttindepbuilderOLSTpq(tt0{j},tt1{j},T(j),xaddon{j},tt1p,tt0p,pT,m{j},m0{j},phetrgn{j},qhetrgn{j}) ; % drop last value
        NV3{j}           = ttr1addon-IVttind1addon(1:T(j),:)*pqalphatempaddon;
        Fmu{j}           =   zeros(T(j),2,3);             %  the first two columns are one
        Fmu{j}(:,1,1)    =   ones(T(j),1);
        Fmu{j}(:,2,2)    =   ones(T(j),1);
        Fmu{j}(:,:,3)    =  repmat(betaaddon(2:3)',[T(j) 1]).* repmat(alphaMaddon.*[tt0p;tt1p(pT-T(j)+1:pT-1)']-[tt0{j};tt1{j}(1:T(j)-1)'],[1 2]);
        ymu             =   [NV1{j}  NV2{j} NV3{j}];
...
%         uT = uTmu{j};
        ...
        [mmu{j},Cmu{j},m0mu{j},C0mu{j},tt1mu{j}] = kalffbsMeasurementVersion(ymu,Fmu{j},2,m0mu{j},C0mu{j},sigmaiv{j},wmu{j},GTmu{j},uTmu{j});

        %update the data
        phetrgn{j}(:,2:3) = tt1mu{j}(:,2:T(j)+1)'; %replace for the next iteration of EKF

        % estimate non-state parameter of System equation of version        
         X1 =[ones(T(j),1) tt1mu{j}(1,1:T(j))'];
         X2 = [ones(T(j),1) tt1mu{j}(2,1:T(j))'];
         Y11 = tt1mu{j}(1,2:(T(j)+1))';
         Y21 = tt1mu{j}(2,2:(T(j)+1))';
         S11   =  ((X1'*X1)/w1{j}+smuInv)\speye(size(((X1'*X1)/w1{j}+smuInv))); 
         S12   =  ((X2'*X2)/w2{j}+smuInv)\speye(size(((X2'*X2)/w2{j}+smuInv))); 
         b11   =  S11*(((X1'*Y11)/w1{j})+smuInv*meanmu); %btempp(j,1)
         b12   =  S12*(((X2'*Y21)/w2{j})+smuInv*meanmu); %btempp(j,1)
         gammamu1   =  b11 + chol(S11)'*randn(length(b11),1);
         gammamu2   =  b12 + chol(S12)'*randn(length(b12),1);
         GTmu{j}(1,:)=gammamu1;
         GTmu{j}(2,:)=gammamu2;
         uTmu{j} = GTmu{j}(:,1) ;

         %estimate variance wmu (System equation of version)
         wiwmu = v0iv + T(j);
         ewmu1 = Y11 - X1*gammamu1;
         ewmu2 =  Y21 - X2*gammamu2;
         wsigmaiwmu = inv(diag([0.5 0.5])+[ewmu1 ewmu2]'* [ewmu1 ewmu2]); %I used the prior directly
         wmu{j} = inv(wishrnd(wsigmaiwmu,wiwmu));
         w1{j} = wmu{j}(2,2) -wmu{j}(2,1)/wmu{j}(1,1)*wmu{j}(1,2);
         w2{j} = wmu{j}(1,1) -wmu{j}(1,2)/wmu{j}(2,2)*wmu{j}(2,1);
         corrstateeq = corr(ewmu1,ewmu2);
         
         %-----------------------------------------------------------------
         % control function covariance portion
         %-----------------------------------------------------------------

        %simulate w ~ IW (System Equation)
    %     f = -qpparam./(M0param+eta*caddon(1:(size(yc,1)-1),:)).*(2*ttind1p.*[m0p mp(:,1:pT-1)]'-[m0p mp(:,1:pT-1)]'.^2) + (1+qpparam-ppparam).*ttind1p-(yc(1:(size(yc,1)-1),:)*npparam').*ttind1p+ppparam*(M0param+eta*caddon(1:(size(yc,1)-1),:)) + (yc(1:(size(yc,1)-1),:)*npparam').*(M0param+eta*caddon(1:(size(yc,1)-1),:));
        f = Fmu{j}(:,1,3).*tt1mu{j}(1,1:T(j))'+Fmu{j}(:,2,3).*tt1mu{j}(2,1:T(j))';
        ewv = NV3{j}-f;
        %plot(1:pT,TNC2,1:pT,f)
        evmu = [NV1{j} NV2{j}]-tt1mu{j}(1:2,2:(T(j)+1))';
        %plot(1:pT,TNC,1:pT,tt1mu(2:pT+1)')
        % plot(1:pT,TNC,1:pT,yc(2:pT+1,3))

        viwmu = v0iv + T(j);
        sigmaiwmu = inv(Sviv+[evmu ewv]'* [evmu ewv]);
        % draw from IW(sigmaiwmu,viwmu)
        sigmaivinv = wishrnd(sigmaiwmu,viwmu);
        sigmaiv{j} = inv(sigmaivinv);

        w(j) = sigmaiv{j}(3,3) -sigmaiv{j}(3,1:2)/sigmaiv{j}(1:2,1:2)*sigmaiv{j}(1:2,3);
        vmu  = [sigmaiv{j}(1,1)  - sigmaiv{j}(1,2:3)/sigmaiv{j}(2:3,2:3)*sigmaiv{j}(2:3,1);...
            sigmaiv{j}(2,2)  - sigmaiv{j}(2,[1 3])/sigmaiv{j}([1 3],[1 3])*sigmaiv{j}([1 3],2)];

        disp('vp and wp for diffusion equation is:')
        disp([v(j); w(j)])
        disp('vmu for measurement equation is:')
        disp(vmu)
        disp('wmu for measurement equation is:')
        disp(wmu{j})

        corrmuobsttst = [corr(ewv,evmu(:,1));corr(ewv,evmu(:,2));corr(evmu(:,1),evmu(:,2))];
        %----------------------------------------------------
        % portion of old code to revise
        %----------------------------------------------------
...
%             0.5*T(j)*p*log(2*pi*w(j)); % LLtemp (j)
            ...
            ivp_{j}(:,jp) = [reshape(nonzeros(triu(sigmaiv{j})),[6 1]);corrmuobsttst];
            cmu_{j} (:,jp)   = [GTmu{j}(:);vmu;reshape(nonzeros(triu(wmu{j})),[3 1]);corrstateeq];
            bmu_{j}(:,jp)    = tt1mu{j}(2:(T(j)+1))';
            b0mu_{j}(:,jp)   = tt1mu{j}(1);
            LLtemp (j)      = LLtemp (j) - 0.5*trace(inv(sigmaiv{j})*[evmu ewv]'* [evmu ewv]) - 0.5*3*T(j)*log(2*pi) - 0.5 * T(j)* det(sigmaiv{j});

            %likelihood portion for new state space for version
            LLtemp (j)      = LLtemp (j) - 0.5*(Y11-X1*gammamu1)'*(Y11-X1*gammamu1)/w1{j} -  0.5*T(j)*log(2*pi)- 0.5*T(j)*(w1{j});
            LLtemp (j)      = LLtemp (j) - 0.5*(Y21-X2*gammamu2)'*(Y21-X2*gammamu2)/w1{j} -  0.5*T(j)*log(2*pi)- 0.5*T(j)*(w2{j});
...
for  j=1:n;
    ...
    %---------------------------------------------------------------------
    % updated portion for IV of the new version
    %---------------------------------------------------------------------
    % fill the latent iv
    tt1mu{j}(1) = mean( b0mu_{j}, 2);
    tt1mu{j}(2:(2+1)) = mean(bmu_{j},2);
    phetrgn{j}(:,2:3) = tt1mu{j}; %replace for the next iteration of EKF
    pqalphatempaddon = [-gammaaddonLL{j}*(1-deltaaddonLL{j})/alphaMaddonLL{j};1-deltaaddonLL{j};-betaaddonLL{j}(1);alphaMaddonLL{j}.*betaaddonLL{j}(1);(1-deltaaddonLL{j})*gammaaddonLL{j}];    
    IVttind1addon= addonttindepbuilderOLSTpq(tt0LL{j},tt1LL{j},T(j),xaddon{j},[ttind1pLL(1);ttr1LL],ttind1pLL(1),pT,mLL{j},m0LL{j},phetrgn{j},qhetrgn{j}) ; % drop last value
    ttr1addon        =  tt1LL{j}';        
    NV3{j}           = ttr1addon-IVttind1addon(1:T(j),:)*pqalphatempaddon;
    Fmu{j}           =   zeros(T(j),2,3);             %  the first two columns are one
    Fmu{j}(:,1,1)    =   ones(T(j),1);
    Fmu{j}(:,2,2)    =   ones(T(j),1);
    Fmu{j}(:,:,3)    =  repmat(betaaddonLL(2:3),[T(j) 1]).* repmat(alphaMaddonLL.*ttind1pLL-[tt0{j};tt1{j}(1:T(j)-1)'],[1 2]);
    ymu              =   [NV1{j}  NV2{j} NV3{j}];
    f = Fmu{j}(:,1,3).*tt1mu{j}(1,1:T(j))'+Fmu{j}(:,2,3).*tt1mu{j}(2,1:T(j))';
    ewv = NV3{j}-f;
    evmu = [NV1{j} NV2{j}]-tt1mu{j}(1:2,2:(T(j)+1))';  
    ...
    Y11 = tt1mu{j}(1,2:(T(j)+1))';
    Y21 = tt1mu{j}(2,2:(T(j)+1))';
    sigmaiv{j} = triu(ones(3),0);
    sigmaiv{j}(~~sigmaiv{j}) = mean(ivp_{j},2);
    tempd = diag(sigmaiv{j});
    sigmaiv{j}=sigmaiv{j}+ sigmaiv{j}';
    sigmaiv{j}(1:3+1:3*3)=tempd;
    cmuMean  =  mean(cmu_{j},2);
    GTmu{j}  = cmuMean(1:4);
    gammamu1=  GTmu{j}(1,:);
    gammamu2=     GTmu{j}(2,:);
    wmu{j}   = triu(ones(2),0);
    wmu{j}(~~wmu{j})  = cmuMean(5:7);
    tempd = diag(wmu{j});
    wmu{j} = wmu{j} + wmu{j}';
    wmu{j} (1:2+1:4) = tempd;
    w1{j} = wmu{j}(2,2) -wmu{j}(2,1)/wmu{j}(1,1)*wmu{j}(1,2);
    w2{j} = wmu{j}(1,1) -wmu{j}(1,2)/wmu{j}(2,2)*wmu{j}(2,1);         
    LLmean      = LLmean - 0.5*trace(inv(sigmaiv{j})*[evmu ewv]'* [evmu ewv]) - 0.5*3*T(j)*log(2*pi) - 0.5 * T(j)* det(sigmaiv{j});
    %likelihood portion for new state space for version
    LLmean      = LLmean - 0.5*(Y11-X1*gammamu1)'*(Y11-X1*gammamu1)/w1{j} -  0.5*T(j)*log(2*pi)- 0.5*T(j)*(w1{j});
    LLmean      = LLmean - 0.5*(Y21-X2*gammamu2)'*(Y21-X2*gammamu2)/w1{j} -  0.5*T(j)*log(2*pi)- 0.5*T(j)*(w2{j});
...
for i=1:n
    ...
    % HPDI
    [n1,n2]=size(ivp_{i});
     hpdismu{i}=zeros(n1,2);
      for ii=1:n1
         hpdismu{i}(ii,1:2) = hpdi(ivp_{i}(ii,:)',.95);
      end
      hpdismu{i};
end;
