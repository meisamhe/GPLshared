...
wcat[is.nan(wcat)] = 10
ewcat = t(chol(make.positive.definite(wcat)))%*%matrix(rnorm(ncat*T,mean=0,sd=1),ncol=T)
...
catEKF= function(ycat,Fcat,pcat,m0cat,C0cat,vcat,wcat,thetacatj) {
  ...
  tt1cat[,T] = mcat[,T]+as.vector(t(chol(make.positive.definite(Ccat[,,T])))%*%as.matrix(rnorm(pcat*2)))
  ...
  for (t in (T-1):1){
    ...
    tt1cat[,t]  = tmcat     + t(chol(make.positive.definite(Cmcat)))%*%as.matrix(rnorm(2*pcat)) 
    tt1cat[,t]= pmin( tt1cat[,t],maxDiff)
    tt1cat[,t]= pmax(0, tt1cat[,t])                                                                                  
  }
  ...
  tt0cat    =m0cat     + t(chol(make.positive.definite(C0cat)))%*%as.matrix(rnorm(2*pcat))
...
drawDelta=
  function(x,y,z,comps,deltabar,Ad){
    ...
    cov=ginv(make.positive.definite(xtx+Ad))
    cov = (cov+t(cov))/2
    cov = ginv(ginv(cov))
    return(cov%*%(xty+Ad%*%deltabar) + t(chol(make.positive.definite(cov)))%*%rnorm(length(deltabar)))
  }
...
basslikelihoodCat= function(thetacatjestcur,thetacatjest,tt0cat,tt1cat,wcat,oldcompthetacatj,indthetacatj,curcat,meansuntilcur,betabarthetacatj){ 
  ...
  thetacatjest[curcat,5] = pmin(thetacatjest[curcat,5],300)
  thetacatjest[curcat,6] = pmin(thetacatjest[curcat,6],300)
  thetacatjest[curcat,7] = pmin(thetacatjest[curcat,7],300)
...
segmentLikelihood= function(threshold,ycat, tt1cat,vcat,curcat,betabarthetacatj,oldcompthetacatj,indthetacatj){ 
  ncat =  dim(tt1cat)[1]/2
  T    =  dim(tt1cat)[2]
  #truncate to make sure it is relevant
  threshold = pmin(threshold,300)
...
for(iterrep in 1:R){
  ...
  for (clt in 1:ncat){
    ...
    if (is.positive.definite(hessiancatTemp)){
      varcattemp  = diag(chol2inv(chol(make.positive.definite(hessiancatTemp))))
    }else{
  ...
  thresholds = pmin(thresholds,300)
  ...
  llcatObsEqCur =  - 0.5*sum(diag(make.positive.definite(vcatTemp))%*%crossprod(errortempcat,errortempcat))-
