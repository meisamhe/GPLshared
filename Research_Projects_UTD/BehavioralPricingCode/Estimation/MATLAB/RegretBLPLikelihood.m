...
function y = RegretBLPLikelihood(x)
...
betar    =  x(1,4); %stock out regret difference heterogeneity coeff
...
%disp(shares_n)
%pause
% including cost to explain consumption utility
p=5;
X1= [ones(J,1).*cost (0.5*Dur1+gamma.*Dur2) P1 lambda.*(P1-P2) zeros(J,1)];
X2= [gamma.*lambda.*cost 0.5*gamma.*lambda.*Dur2 gamma.*lambda.*P2 zeros(J,1) gamma.*(ones(J,1)-lambda).*(0.5*Dur1+gamma.*Dur2)];
X=[X1 X2]';
Xn=reshape(X,p,J*2);
Xn=Xn'; %stack X's
Yn= shares_n;
