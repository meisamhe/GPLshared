...
alpha  =    x(1,5:J+4)'; % brand fixed effect

%simple minimum distance estimator
uij1 =      alpha + (c + gamma*c)*ones(J,1) + betap(1,1)*P1 + alphap(1,1)*lambda.*(P1-P2);
uij2 =      gamma*(lambda.*(alpha+c*ones(J,1)+betap(1,1)*P2) + betar(1,1)*(1-lambda).*log(ones(J,1) + exp(alpha+(c+gamma*c)*ones(J,1)+betap(1,1)*P1)));
...
SE=(Sh-esh).^2;
variance                 =    mean(SE(:));
LogJacobian              =    - sum(sum(-log(esh.*(ones(J,3) -   esh))));
