...
%X0 = [-1 -1 -1 1];
%X0= [-0.5 -0.1 -0.5 -0.5];
X0=[-5.7020    0.3109   -2.5070    1 2.2127];
options=optimset('Display','on','MaxIter',10000,'TolX',10^-30,'TolFun',10^-30,'UseParallel','always');
[x,fval,exitflag,output,grad,hessian]=fminunc('FuncIdentificationBLPKhanTravrskWithRegEffcntData',X0,options);
...
disp([exp(x(1,1))/(1+exp(x(1,1))) x(1,2:4) betarh_e])
...
FitnessFunction = @FuncIdentificationBLPKhanTravrskWithRegEffcntData;
numberOfVariables = 5;
