function y = FuncMeanUtilityEst(x,delta,P1,P2,lambda,gamma,J)
    global likelihoodM nonlinearp betarp linearp;
    ...
    e22   =     e2.^2;
    betarp   =   betar;
    nonlinearp   =   alpha+(c+gamma*c)*ones(J,1)+betap*P1;
    linearp = lambda.*(alpha+c*ones(J,1)+betap*P2);
    % Likelihood function
    SE    =    sum(e12 + e22);
    variance                 =    mean(SE(:));
    likelihoodM =    - J*3*log(sqrt(2*pi*variance)) -   sum(sum(SE./variance));
    y           =    - likelihoodM;