...
function y = FuncIdentificationBLPWithRegEffcntData(x)
...
while(k>km)
    ...
    uij2s2 =      de(Indx,2)+gamma(Indx).*(lambda(Indx).*(bpd*P2(Indx))+ tth1d*(ones(size(Indx,2),1)-lambda(Indx)).*(0.5*Dur1(Indx)+gamma(Indx).*Dur2(Indx))); 
