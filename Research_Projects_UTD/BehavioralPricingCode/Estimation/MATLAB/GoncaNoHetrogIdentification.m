...
% calculate utility
uij1 =      alpha+(0.5*Dur1+gamma.*Dur2).*c+bp*P1+ xi(:,1);
uij2 =      gamma.*(lambda.*(alpha+0.5*c.*Dur2+bp*P2))+ xi(:,2);
...
%beta=[alpha c bp];
X1= [ones(J,1) (0.5*Dur1+gamma.*Dur2).*ones(J,1) P1 ];
X2= [gamma.*lambda gamma.*lambda.*Dur2.*0.5 gamma.*lambda.*P2];
