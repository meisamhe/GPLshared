...
pi1    = 0.8;%0.0000000001;%0.9999999;%0.99999;%rand(1,1); %size of first segment
bp(1,1)    = -rand(1,1);
bpd   = -.9*rand(1,1);
bp(1,2)    = bp(1,1)+ bpd;
% high price regret coefficient
alphap(1,1) = -5.2*rand(1,1);
alphapd = -2.2*rand(1,1);
...
[x,fval,exitflag,output,grad,hessian]=fminunc('FuncIdentificationBLP',X0,options);
...
FitnessFunction = @FuncIdentificationBLP;
numberOfVariables = 4;
[x,fval] = ga(FitnessFunction,numberOfVariables);
[exp(x(1))/(1+exp(x(1))) x(2:3)]

%Likelihood at the point
X0=[log(pi1/(1-pi1)) bpd alphapd betard*c]; %starting from correct point
FuncIdentificationBLP(X0)
