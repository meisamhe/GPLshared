...
function y = FuncIdentificationBLPKhanTravrskWithRegEffcntData(x)
...
alphadd   = x(4);
tth1d     =   x(5); %stock out regret difference heterogeneity coeff multiplied to consumption utility

dd       =  zeros(J,T); %deltatemp*.5;%deltatemp/2; 
...
while(k>km)
    ...
    if (ceil(i./1000) == (i./1000))|| (i==1)
        ...
        k
        disp(x)
        if (i>3000000) || (floor(k) ==floor(114)) % for genetic algorithm
            penalty = -Inf;
            break;
        end;
    ...
    uij1s2 =      de(Indx,1)+bpd*P1(Indx) + alphapd.*lambda(Indx).*(P1(Indx)-P2(Indx));    
    uij2s2 =      de(Indx,2)+gamma(Indx).*(lambda(Indx).*(bpd*P2(Indx))+ alphadd.*lambda(Indx).*(P1(Indx)-P2(Indx)) + tth1d*(ones(size(Indx,2),1)-lambda(Indx)).*(0.5*Dur1(Indx)+gamma(Indx).*Dur2(Indx))); 
...
%beta=[alpha c bp];
p = 6;
X1= [zeros(J,1) cost (0.5*Dur1+gamma.*Dur2) P1 lambda.*(P1-P2) zeros(J,1)];
