...
uij2s1 = gamma.*(availlambda.*(alphai+Dur2/2.*tti-bp1*P2)+(1-availlambda).*betar1.*max(alphai+ (Dur1/2+gamma.*Dur2).*tti + bp1*P1,0))+xi(:,2);
% utility of second segment in the first period
uij1s2 = (alphai+ (Dur1/2+gamma.*Dur2).*tti + bp2*P1) + alphap2*availlambda.*(P1-P2) + xi(:,1);
uij2s2 = gamma.*(availlambda.*(alphai+Dur2/2.*tti-bp2*P2)+(1-availlambda).*betar2.*max(alphai+ (Dur1/2+gamma.*Dur2).*tti + bp2*P1,0))+xi(:,2);
...
Jacobian=max(Jacobian,1e-160);   %As a precaution

LogJacobian              =    -sum(log(Jacobian(:)));


LogDemandShockLikelihood =    - 0.5*T*J*log(2*pi*variance) -   0.5*sum(xi(:).^2/variance);
