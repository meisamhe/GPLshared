...
pi1    = 0.4;%0.999;%0.8; %0.001;%0.0000000001;%;%0.99999;%rand(1,1); %size of first segment
bp(1,1)    = -3*rand(1,1);
bpd   = -4*rand(1,1);
bp(1,2)    = bp(1,1)+ bpd;
...
shares=[exp(-log(1+e1+e2))*[pi1;1-pi1] exp(log(e2)-log(1+e1+e2))*[pi1;1-pi1]];
...
if (sum(sum(isinf(log(1+e1+e2))))>0)
    ...
    shares=[exp(log(e1)-log(1+e1+e2))*[pi1;1-pi1] exp(-log(1+e1+e2))*[pi1;1-pi1]];
    %period 1, segment 1, segment 2
...
outside = repmat(1-sum(shares,2),[1 2]);
...
delta =deltareal(:)';
X0=[log(pi1/(1-pi1)) bpd alphapd tth1d delta]; %starting from correct point
...
delta =deltareal(:)';
X0=[log(pi1/(1-pi1)) bpd alphapd betard*c delta]; %starting from correct point
%X0 = xresultGA;
...
options = optimset(options,'MaxIter',1e15);
[xnew,fval,exitflag,output,lambda,grad,hessian] =fmincon(@(x)0,X0,[],[],[],[],lb,ub,'shareconstraint',options);
X0 = xnew;
%[ce,ceq]=shareconstraint(X0)
%setting options
options  =  optimset('fmincon');
options = optimset(options,'Algorithm','interior-point','Display','Iter','TolCon',1e-13,'TolFun',1e-13,'TolX',1e-13);
