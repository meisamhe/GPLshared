...
submission = "C:\\Users\\mxh109420\\Desktop\\KaggleCometition\\submission02042015MHEDNNTTstBg0.1,1,18,18,Bg3NoFilterInteraction.csv"  # path of to be outputted submission file
...
L1 = 18     # L1 regularization, larger value means more regularized
L2 = 18    # L2 regularization, larger value means more regularized
...
class ftrl_proximal(object):
    ...
    def predict(self, x):
        ...
        wTx = 0.
        #print('maximum of z is:%s'%(max(z)))
        #print('minimum of z is:%s'%(min(z)))
        for i in x:
...
def data(path, D):
	...
	for t, row in enumerate(DictReader(open(path))):
		...
		for i in ['hour','C1','banner_pos','site_category','app_category','device_type','device_conn_type','C15','C16','C18','C19','C20','C21']:
			for j in ['hour','C1','banner_pos','site_category','app_category','device_type','device_conn_type','C15','C16','C18','C19','C20','C21']:
				if (i!=j):
					i1 = row[i]
					j1 = row[j]
					keyvalue = i+j+i1+j1
					if keyvalue in dictionaryFeature:
						index = dictionaryFeature[keyvalue]
						x.append(index)
...
for i in ['hour','C1','banner_pos','site_category','app_category','device_type','device_conn_type','C15','C16','C18','C19','C20','C21']:
	for j in ['hour','C1','banner_pos','site_category','app_category','device_type','device_conn_type','C15','C16','C18','C19','C20','C21']:
		if (i!=j):
			Elementsi = basicStat[i]
			Elementsj = basicStat[j]
			for i1 in Elementsi:
				for j1 in Elementsj:
					dictionaryFeature[i+j+i1+j1]=D
					D = D + 1
