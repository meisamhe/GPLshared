...
submission = "C:\\Users\\mxh109420\\Desktop\\KaggleCometition\\submission02062015MHELHSSparseKNN5000Buck.csv"  # path of to be outputted submission file
...
#create vectors of count (*32 to make sure it is not small)
countVec = [0]*D*32
choiceVec = [0]*D*32
...
with open(submission, 'w') as outfile:
	...
	for t, date, ID, x, y in data(test, D):
		...
		switch = False
		while switch==False:
			lower = max(0,bucket - window)
			upper = min(D*32,bucket + window)
			totalCount = 0
			totaly = 0
			for i in range(lower,upper,1):
				totalCount += countVec[i]
				totaly += choiceVec[i]
			if (totalCount>minBucketSz):
				switch= True
				p = float(totaly)/totalCount
			else:
				window *= 4
