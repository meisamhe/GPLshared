...
#submission = "C:\\Users\\mxh109420\\Desktop\\KaggleCometition\\submission1234ActualFeatureRemovedNoiseNoTest.csv"  # path of to be outputted submission file
trainPrefix = "C:\\Users\\mxh109420\\Desktop\\KaggleCometition\\train\\train"
csvPostfix = ".csv"
testPrefix = "C:\\Users\\mxh109420\\Desktop\\KaggleCometition\\test\\test"
...
# number of training split 20
ts = 20
outfileTrain = list()
for i in range(1,ts+1,1):
	fileName=trainPrefix+'%s'%i+csvPostfix
	outfileTrain.append(open(fileName, 'w') )

header = dictionaryFeature.keys()
for e in xrange(epoch):
    loss = 0.
    count = 0
    for currentFile in range(0,ts,1):
		outfileTrain[currentFile].write('"i" "j" "x"\n')
		#outfileTrain[currentFile].write('prediction')
		#for i in range(0,D,1):
		#	outfileTrain[currentFile].write(',%s' % (header[i]))
		#outfileTrain[currentFile].write('\n')
    for t, date, ID, x, y in data(train, D):  # data is a generator
		...
		# select current File to save on 
		currentFile = random.randint(1, ts-1)
		...
		# write data in the sparse model
		outfileTrain[currentFile].write('%s %s %s\n' % (counterTest,1,y))
		#outfileTrain[currentFile].write('%s' % (y))
		for i in range(0,D,1):
			#outfileTrain[currentFile].write(',%s' % (x[i]))
			# because the first element is y => i+1
			if (x[i]==1):
				outfileTrain[currentFile].write('%s %s %s\n' % (counterTest,i+2,x[i]))
		#outfileTrain[currentFile].write('\n')
...
##############################################################################
counterTest = 0
# number of test split 2
ts = 3
outfileTrain = list()
for i in range(1,ts+1,1):
	fileName=testPrefix+'%s'%i+csvPostfix
	outfileTrain.append(open(fileName, 'w') )
...
for currentFile in range(0,ts,1):
		# different from training because here we do not have the choice but we have the ID
		outfileTrain[currentFile].write('"i" "j" "x"\n')
		#outfileTrain[currentFile].write('ID')
		#for i in range(0,D,1):
		#	outfileTrain[currentFile].write(',%s' % (header[i]))
		#outfileTrain[currentFile].write('\n')
...
for t, date, ID, x, y in data(test, D):
	...
	# write data in the sparse model
	outfileTrain[currentFile].write('%s %s %s\n' % (counterTest,1,ID))
	#outfileTrain[currentFile].write('%s' % (ID))
	for i in range(0,D,1):
		#outfileTrain[currentFile].write(',%s' % (x[i]))
		# because the first element is y => i+1
		if (x[i]==1):
