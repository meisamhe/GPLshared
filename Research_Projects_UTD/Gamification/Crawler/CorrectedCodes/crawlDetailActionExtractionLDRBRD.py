...
for i in range(0,numUsers,1):
	...
	while (missionFulfilled == False):
		...
		if len(acceptedDates) != 0:
			for i in acceptedDates:
				...
				if first == True:
					...
					if (float(dateDecom[0]) <= float(minDate[0])) and (float(dateDecom[1]) <= float(minDate[1])) and (float(dateDecom[2]) <= float(minDate[2])):
						minDate = dateDecom
		...
		if (float(minDate[0]) <= 2014) and (float(minDate[1]) <= 6) and (float(minDate[2]) <= 22):
					missionFulfilled = True
		
		if (len(nextPage)!=0) and (missionFulfilled==False):
			urlTail = nextPage[0]
			urlTail = urlTail.replace("&amp;", "&");
			url='http://stackoverflow.com'+urlTail
