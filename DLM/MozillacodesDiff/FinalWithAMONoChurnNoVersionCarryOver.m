% Final Robustness check on May 22, remove carry over of version
...
if (size(brkpnttemp,1)>0)
   if (size(brkpnttemp,1)>1)
       for i=1:size(brkpnttemp,1)-1
           vertemp(brkpnttemp(i):brkpnttemp(i+1)-1) = carryover.^[1:brkpnttemp(i+1)-brkpnttemp(i)];
       end;
   end;
   i = size(brkpnttemp,1);
   vertemp(brkpnttemp(i):size(vertemp,1)) = carryover.^[1:size(vertemp,1)-brkpnttemp(i)+1];
   ffxvrsn  = vertemp;
end;
% 
...
pqM0betaNewtemp  = zeros(parallelsize,kp);
 ...
 C0p = repmat(eye(pp),[1 1]);
  ...
  T = pT;
  tt1p    =  15*ones(p,pT); 
  vp     =  0.017; % it includes all observation equations of platform state equation
  wp = 0.1076;              % known observation and sytem variance
...
%             disp('once.....................');
           ...
           postPNewaddon  = NoChurnMHLLSROLRTpq(pqalphaaddonNew,ttind1addon,ttr1addon,T(j),w(j),b0(:,j),etacs,rp,rq); % log of new posterior
           postPOldaddon  = NoChurnMHLLSROLRTpq(pqalphaaddon{j},ttind1addon,ttr1addon,T(j),w(j),b0(:,j),etacs,rp,rq); % log of old posterior
...
%         end;
        deltaparamtemp  = 0;
        %CSd(j,:)*alphacsd(:,1);%; CSINT(j,:)*alpha(6)]; ;CSST(j,:)*alphacs(6)
