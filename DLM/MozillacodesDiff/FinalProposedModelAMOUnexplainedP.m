...
% pitmes = [3,4,5,6,7,11,12];
 ...
 %for the case of unexplained heterogeneity
 CS1Inv = 1.5e-7*eye(1);
 cs1    =  zeros(1,1);
...
%         disp([alphaMaddon betaaddon' gammaaddon']) %pparamaddon qparamaddon
   ...
   % estimation of plug in level effects
    ...
    S1   =  (((CSp0'*CSp0)/etacsp(j))+CS1Inv)\speye(size((((CSp0'*CSp0)/etacsp(j))+CS1Inv))); %inv(((CSL'*CSL)/eta(j))+CS0Inv); 
    b1   =  S1*(((CSp0'*btempaddon(j,:)')/etacsp(j))+CS1Inv*cs1); %btempp(j,1)
    ...
    j=2; % addon version
    S1   =  (((CSp1'*CSp1)/etacsp(j))+CS1Inv)\speye(size((((CSp1'*CSp1)/etacsp(j))+CS1Inv)));% inv(((CSIntr'*CSIntr)/eta(j))+CS0Inv); 
    b1   =  S1*(((CSp1'*btempaddon(j,:)')/etacsp(j))+CS1Inv*cs1); % btempp(j,1)
    ...
    j=3; % ffox version
    S1   =  (((CSp2'*CSp2)/etacsp(j))+CS1Inv)\speye(size((((CSp2'*CSp2)/etacsp(j))+CS1Inv)));% inv(((CSSQ'*CSSQ)/eta(j))+CS0Inv); 
    b1   =  S1*(((CSp2'*btempaddon(j,:)')/etacsp(j))+CS1Inv*cs1); %btempp(j,1)
