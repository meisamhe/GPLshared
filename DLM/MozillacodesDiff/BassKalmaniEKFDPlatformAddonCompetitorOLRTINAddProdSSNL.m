...
%============================ Data ======================================
%==========================================================================
rp     = 4; % since here it is OL, Rating, version, weekend [only include weekend]
rq    = 5; % since both variance of rating and observational learning to explain heterogeneity in q
ssn   = 1; % to capture seasonal effect in observation equation
k     =  2+rp+rq; % alpha + delta+ terms for p terms for q
jumps      =    1; idx = 0; jp = 0; ndraw=10000;  ndraw0 = 4000; 
...
crtdAddons =num(:,34)/scale; %10 million firefox platform download [Scale in order to avoid singularity]
crtdAddons = cumsum(crtdAddons); % since cumulative reviews are important
updtdAddons =num(:,32)/scale; %10 million firefox platform download [Scale in order to avoid singularity]
...
parfor i=1:n;
  ...
  %  T(1,i)=min(min(checksize(num(:,15)),checksize(num(:,1))),checksize(num(:,8)));
   ...
   % beta = 0.0001*rand(1,1);
    [xaddon{i},y{i},uT{i},ol{i},st{i},ver{i},wkend{i},status{i},T(i),stol{i}] = readOLRTSSNL(numaddon,T(i),alphabetaapram{i},p,tt1p,tt0p,pT,scale,pusg,ffxvrsn); %int{i},piai{i}
    phetrgn{i} =[ones(T(i),1) ver{i} ffxvrsn(pT-T(i)+1:pT) ver{i}.*ffxvrsn(pT-T(i)+1:pT) ];% wkend{i}% int{i} %wkend{i}  ver{i} wkend{i}  ver{i} wkend{i} ver{i}
    qhetrgn{i} =[ones(T(i),1) st{i} ol{i} stol{i} status{i}]; %ol{i} st{i} ol{i} status{i}
    Xi{i}=1e-6;
    y{i}=y{i}-Xi{i}*wkend{i};
  disp(i);
  y{i} = cumsum(y{i});
  disp('done...');
end;
...
CSXWD =zeros(n,csk);
 ...
 % seasonal effect capturing
 yb0    = cell([1 n]); %[0.5; 0.5; .5; .5]; 
 YS0Inv = 1.5e-7*eye(1);
 ...
 alphacs=zeros(csk,k); 
 btemp=zeros(k,n);
 dcs_=zeros(k*csk+k+ssn*2,((ndraw-ndraw0)/jumps)); 
 ...
 pcaddon{1}= 5e-4;
 for i=2:n;
     pcaddon{i}=4e-3; %tunning parameter
 end;
   pcaddon{19}= 7e-3;
   pcaddon{26}= 5e-4;
   pcaddon{28}= 5e-3;
...
for i = 1:ndraw
   ...
   % simulate observation equation parameter (which is state equation for
    ...
    parfor j=1:n;
        ...
        y{j}=y{j}+Xi{j}*wkend{j};% recover real value
        S1   =  inv(((wkend{j}'*wkend{j})/v(j))+YS0Inv);
        b1   =  S1*(((wkend{j}'*(y{j}' - ttr1addon')')/v(j))+YS0Inv*yb0{j});
        Xi{j} =  b1 + chol(S1)'*randn(length(b1),1);
        Xitemp(j) = Xi{j};
        y{j}=y{j}-Xi{j}*wkend{j}; % prepare for next ffbs
        
        va1= (va0+T(j))/2;
        vv12= (vv02+(y{j}' - ttr1addon' - Xi{j}*wkend{j}')*(y{j}' - ttr1addon' - Xi{j}*wkend{j}')')/2;
...
%         qparamaddon     = pqalphaaddon{j}(2);
      ...
      %  piai{j} = pparamaddon*alphaMaddon; % to use in the hap function   
        ...
        if (sw==1);
            ...
            b0_{j}         =     tt0{j}; 
            LLtemp (j)      =  LLtemp (j)-0.5*(y{j}' - ttr1addon' - Xi{j}*wkend{j}')*(y{j}' - ttr1addon' - Xi{j}*wkend{j}')'/v(j).^2 - ...
            0.5*T(j)*p*log(2*pi*v(j).^2);
            errv_{j}  =  errv_{j}+(y{j}' - ttr1addon')';
           c_{j}          =     [btempaddon(:,j);Xi{j} ;v(j);w(j)];
...
%     etacs(j)   =  1/gamrnd(wa1,1/wv12);   
% %    
    % explaining weekend dummy
    S1   =  inv(((CSXWD'*CSXWD)/etaw)+CS0Inv);
    b1   =  S1*(((CSXWD'*Xitemp')/etaw)+CS0Inv*csb0);
    WDC   =  b1 + chol(S1)'*randn(length(b1),1);
    wa1  = (wa0+n)/2;
    wv12 = (wv02+(Xitemp'-CSXWD*WDC)'*(Xitemp'-CSXWD*WDC))/2;
    etaw   =  1/gamrnd(wa1,1/wv12);
...
%         qparamtemp     = CSIntr(j,:)*alphacs(:,3);
    ...
    if (sw==1)
         dcs_(:,jp)          =     [alphacs(:);WDC;etacs';etaw];
    end;
...
% add-on parameters
c_temp=zeros(k+2+ssn,(ndraw-ndraw0)/jumps,n);
parfor i=1:(ndraw-ndraw0)/jumps;
    i;
    c_temp(:,i,:)  =  cell2mat(ci_{i});
end;
ce_ = zeros(size(c_temp,1),n);
for i=1:n;
    i;
...
for i=1:52;
    for j=1:size(c_temp,1);
        j_(j,1)=j_(j,1)+i_(size(c_temp,1)*(i-1)+j);
    end;
end;
