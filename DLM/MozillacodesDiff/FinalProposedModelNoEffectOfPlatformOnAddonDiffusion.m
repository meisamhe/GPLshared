...
%         errvp_  =  errvp_ + evv;
   ...
   % simulate observation equation parameter (which is state equation for
    ...
    parfor j=1:n;
        ...
        [m{j},C{j},m0{j},C0{j},tt1{j},tt0{j},Y1{j},MAD{j},MSE{j}]     =     BEKFAddonpq(y{j},F,p,m0{j},C0{j},v(j),w(j),GT(:,:,j),uT{j}',j,pT,tt1ptemp,tt0ptemp,phetrgn{j},qhetrgn{j},rp,rq);   %,a,b,alpha,piaitemp(j),mp(:,1:pT),m0p(:,:)
        ...
        ttind1addon= addonttindepbuilderOLSTpq(tt0{j},tt1{j},T(j),xaddon{j},tt1ptemp,tt0ptemp,pT,m{j},m0{j},phetrgn{j},qhetrgn{j}) ; % drop last value
...
%         disp([alphaMaddon betaaddon' gammaaddon']) %pparamaddon qparamaddon
        ...
        uT{j} =   alphaMaddon.*(phetrgn{j}*betaaddon)'.*xaddon{j}.*[tt0ptemp tt1ptemp(pT-T(j)+1:pT-1)]; %uT includes relevant market size in it pparamaddon*alphaMaddon*xaddon{j}.*[tt0p tt1p(pT-T(j)+1:pT-1)]+
...
for  j=1:n;
    ...
    ttind1addonLL= addonttindepbuilderOLSTpq(tt0LL{j},tt1LL{j},T(j),xaddon{j},tt1ptemp,tt0ptemp,pT,mLL{j},m0LL{j},phetrgn{j},qhetrgn{j}) ; % drop last value
