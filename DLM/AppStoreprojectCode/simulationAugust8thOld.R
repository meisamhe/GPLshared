...
#         stop("Negative Variance Found in C Matrix",Call.=FALSE)
	    Cmcat= make.positive.definite(Cmcat)
      }
...
#          Ctapp[a] = rapp[a] - Aapp%*%qapptemp%*%t(Aapp)
	   ...
	   if (is.positive.definite(qapptemp)){
		...
		Aapp = rapp[a]%*%t(Fapp)%*%ginv(qapptemp)
...
#                priorLatentVar = priorLatentVar,distortionIndv = distortionIndv)
   ...
   out             =  optim(latentitold,llmnllatent,method="SANN",control=list( fnscale=-1,trace=1,reltol=1e-3,maxit=2000), #hessian = TRUE
